...
$Applications = Get-WmiObject -ComputerName $SiteServer -Namespace root\SMS\site_$SiteCode -class SMS_ImagePackage
...
if ($AppsToDistribute.Count -ge 1) {
    ...
    $AppsToDistribute | ForEach-Object {
        $ID = $_
        #$CurrentAppName = (Get-WmiObject -Namespace "root\SMS\site_$($SiteCode)" -Class SMS_OperatingSystemInstallPackage -ComputerName $SiteServer | Where-Object { $_.ModelName -like "$($ID)" }).LocalizedDisplayName
        Write-Output "Distributing: $ID"
        Start-CMContentDistribution -OperatingSystemImageId $ID -DistributionPointGroupName $DistributionGroup | Out-Null
    }
}
else {

$SiteServer = "anc-sccm-site01.ua.ad.alaska.edu"
$SiteCode = "049"
$DistributionGroup = "NewDistro"
...
$Applications = Get-WmiObject -ComputerName $SiteServer -Namespace root\SMS\site_$SiteCode -class SMS_BootImagePackage

Write-Host $Applications.PackageID

$Applications | ForEach-Object {
    $AppName = $_.PackageID
    $ModelName = $_.Name
    if ($DPGroupArray -notcontains $AppName ) {
        $AppsToDistribute += $AppName
    }
}

if ($AppsToDistribute.Count -ge 1) {
    Write-Output "Found a total of $($AppsToDistribute.Count) Boot Images to distribute:`n"
    $AppsToDistribute | ForEach-Object {
        $ID = $_
        #$CurrentAppName = (Get-WmiObject -Namespace "root\SMS\site_$($SiteCode)" -Class SMS_BootImagePackage -ComputerName $SiteServer | Where-Object { $_.PackageID -like "$($ID)" }).PackageID
        Write-Output "Distributing: $ID"
        Start-CMContentDistribution -BootImageId $ID -DistributionPointGroupName $DistributionGroup | Out-Null
    }
}
else {

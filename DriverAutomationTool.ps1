...
"@ -IgnoreWarnings | Out-Null
	...
	$ShowHidePlatformButton = New-Object 'System.Windows.Forms.Button'
	...
	$labelSettingsStorageConfi = New-Object 'System.Windows.Forms.Label'
	$SettingsTabs = New-Object 'System.Windows.Forms.TabControl'
	$MainOptionsTab = New-Object 'System.Windows.Forms.TabPage'
	...
	$DriverCleanup = New-Object 'System.Windows.Forms.GroupBox'
	...
	$ProxyOptionsTab = New-Object 'System.Windows.Forms.TabPage'
	...
	$AdminOptionsTab = New-Object 'System.Windows.Forms.TabPage'
	...
	$MainForm_Load = {
		...
		# Initialise Form
		global:Write-CMLogEntry -Value "======== INITIALISING LOG FILE & CHECKING PREREQUISITES ========" -Severity 1
		global:Write-CMLogEntry -Value "Info: Driver Automation Tool version - $ScriptRelease" -Severity 1
		global:Write-CMLogEntry -Value "Info: Log File Location - $LogDirectory" -Severity 1
		global:Write-CMLogEntry -Value "Info: Settings File Location - $SettingsDirectory" -Severity 1
		global:Write-CMLogEntry -Value "Info: Temp File Location - $TempDirectory" -Severity 1
	...
	$MainForm_Shown = {
		...
		global:Write-CMLogEntry -Value "======== Detecting Deployment Platform ========" -Severity 1
		...
		if (((Test-Path -Path $Global:SettingsDirectory\DATSettings.xml) -eq $true) -and ($Global:DATSettingsXML.Settings.DownloadSettings.DeploymentPlatform -eq 'MDT')) {
			...
			global:Write-CMLogEntry -Value "Deployment Platform: MDT - Skipping SCCM Validation" -Severity 1
			ValidateMDT
		}
		elseif (((Test-Path -Path $Global:SettingsDirectory\DATSettings.xml) -eq $true) -and ($Global:DATSettingsXML.Settings.DownloadSettings.DeploymentPlatform -match 'ConfigMgr')) {
			...
			$ProgressListBox.ForeColor = "Black"
			global:Write-CMLogEntry -Value "Deployment Platform: SCCM - Validating ConfigMgr Server Details" -Severity 1
			ConnectConfigMgr
		...
		elseif (((Test-Path -Path $Global:SettingsDirectory\DATSettings.xml) -eq $true) -and ($Global:DATSettingsXML.Settings.DownloadSettings.DeploymentPlatform -match 'Download')) {
			$ProgressListBox.ForeColor = "Black"
			global:Write-CMLogEntry -Value "Deployment Platform: $($Global:DATSettingsXML.Settings.DownloadSettings.DeploymentPlatform)" -Severity 1
		}
		else {
			global:Write-CMLogEntry -Value "======== FIRST TIME RUN DETECTED ========" -Severity 1
			
			# Attempt ConfigMgr Site Code & MP Detection
			global:Write-CMLogEntry -Value "Info: Checking WMI for ConfigMgr SMS_Authority Values" -Severity 1 -SkipGuiLog $true
			try {
				...
				if ($SCCMWMI.CurrentManagementPoint -ne $null) {
					global:Write-CMLogEntry -Value "======== ConfigMgr Site Discovery ========" -Severity 1
					$SiteServerInput.Text = $SCCMWMI.CurrentManagementPoint
					global:Write-CMLogEntry -Value "Info: ConfigMgr WMI Query Results - Site Server (Local MP) Found: $($SiteServerInput.Text)" -Severity 1 -SkipGuiLog $true
					$SiteCodeText.Text = ($SCCMWMI.Name).TrimStart("SMS:")
					global:Write-CMLogEntry -Value "Info: ConfigMgr WMI Query Results - Site Code Found: $($SiteCodeText.Text)" -Severity 1 -SkipGuiLog $true
				...
				ErrorOutput -Message "Error: Unable to query ConfigMgr WMI namespace. Error message: $($_.Exception.Message)" -Severity 2
			}
			
			# Set First Time Demo Mode
			switch ($ConfigMgrDetected) {
				$true {
					global:Write-CMLogEntry -Value "Info: ConfigMgr detected - Running first time demo mode" -Severity 1
				...
				$false {
					global:Write-CMLogEntry -Value "Info: Failed to detect ConfigMgr - Running first time demo mode" -Severity 1
		...
		if ($PSVersionTable.PSVersion.Major -lt "3") {
			global:Write-CMLogEntry -Value "======== COMPATIBILITY ISSUE DETECTED ========" -Severity 3
			ErrorOutput -Message "Error: PowerShell Version Incompatible - Please Update PS Installation" -Severity 3
		}
		...
		if ((Test-Path -Path (Join-Path -Path "${env:ProgramFiles(x86)}" -ChildPath "Microsoft.NET\Primary Interop Assemblies\Microsoft.mshtml.dll")) -eq $false) {
			global:Write-CMLogEntry -Value "======== COMPATIBILITY ISSUE DETECTED ========" -Severity 3
			ErrorOutput -Message "Error: Required .Net Intenret Explorer components missing. Lenovo downloades disabled." -Severity 3
			global:Write-CMLogEntry -Value  "Visual Studio isolated shell components can be downloaded from - https://visualstudio.microsoft.com/vs/older-downloads/isolated-shell/" -Severity 3
			global:Write-CMLogEntry -Value  "No warranties provided, install at your own risk." -Severity 3
		...
		if ($global:RunSilent -eq "True") {
			global:Write-CMLogEntry -Value "Mode: Silent running switch enabled" -Severity 2 -SkipGuiLog $true
			...
			If (($ScriptRelease -ne $null) -and ($ScriptRelease -lt $NewRelease)) {
				global:Write-CMLogEntry -Value "Update Alert: Newer Version Available - $NewRelease" -Severity 2 -SkipGuiLog $true
			}
			...
			If (($ScriptRelease -ne $null) -and ($ScriptRelease -lt $NewRelease)) {
				global:Write-CMLogEntry -Value "Update Alert: Newer Version Available - $NewRelease" -Severity 2 -SkipGuiLog $true
				global:Write-CMLogEntry -Value "Update Alert: Opening New Version Form" -Severity 2 -SkipGuiLog $true
				UpdateAvailable
		...
		if ((Get-ScheduledTask | Where-Object {
			...
			global:Write-CMLogEntry -Value "======== Disabling Scheduling Options - Schedule Exits ========" -Severity 1
	...
	$StartDownloadButton_Click = {
		...
		global:Write-CMLogEntry -Value "Info: Validating all required selections have been made" -Severity 1
		if ($UseProxyServerCheckbox.Checked -eq $true) {
			ValidateProxyAccess -ProxyServer $ProxyServerInput.Text -UserName $ProxyUserInput.Text -Password $ProxyPswdInput.Text -URL $URL
		}
		ValidateSettings
		if ($global:validation -eq $true) {
			...
			ErrorOutput -Message "Error: Please make sure you have made all required selections" -Severity 2 $true
		}
	}
	...
	$ConnectConfigMgrButton_Click = {
		$SiteServer = [string]$SiteServerInput.Text
		$ProgressListBox.ForeColor = "Black"
		global:Write-CMLogEntry -Value "======== Validating ConfigMgr Server Details $(Get-Date) ========" -Severity 1
		ConnectConfigMgr
	}
	...
	$AddToListButton_Click = {
		foreach ($Item in $FullModelListBox.SelectedItems) {
			if ($Item -notin $ProductListBox.Items) {
	...
	$DownloadComboBox_SelectedIndexChanged = {
		...
		if ($DownloadComboBox.Text -eq "BIOS") {
			...
			if ($PlatformComboBox.SelectedItem -eq "Download Only") {
				...
				$ArchitectureComboxBox.Enabled = $true
				$PackagePathTextBox.Enabled = $false
				ConfigMgrOptions -OptionsEnabled $false
			}
			else {
				$PackagePathTextBox.Enabled = $true
				$OSComboBox.Enabled = $true
				$ArchitectureComboxBox.Enabled = $true
				$PackagePathTextBox.Enabled = $true
				ConfigMgrOptions -OptionsEnabled $true
			}
			$LenovoCheckBox.Enabled = $true
	...
	$MainForm_FormClosing = [System.Windows.Forms.FormClosingEventHandler]{
		...
		global:Write-CMLogEntry -Value "======== Cleaning Up Temporary Files ========" -Severity 1
		global:Write-CMLogEntry -Value "Info: Removing Temp Folders & Source XML/CAB Files" -Severity 1 -SkipGuiLog $true
		...
		if ($global:NoXMLOutput -eq $false) {
			Write-XMLSettings
			global:Write-CMLogEntry -Value "Info: Updating DATSettings.XML file" -Severity 1 -SkipGuiLog $true
		}
		...
		if ((Get-ScheduledTask | Where-Object {
			...
			global:Write-CMLogEntry -Value "Info: Updating scheduled DATSettings.XML file" -Severity 1 -SkipGuiLog $true
		}
	}
	...
	$MSTechnetSiteLaunchButton_Click = {
		Start-Process -Path "https://gallery.technet.microsoft.com/scriptcenter/Driver-Tool-Automate-9ddcc010"
	}
	
	$MDMLaunchButton_Click = {
		Start-Process -Path "http://www.scconfigmgr.com/modern-driver-management/"
	}
	...
	$ImportCSVButton_Click = {
		...
		MDTOptions -OptionsEnabled $true
		ConfigMgrOptions -OptionsEnabled $true
		Import-CSVModels
	}
	...
	$ConnectWebServiceButton_Click = {
		if ((![string]::IsNullOrEmpty($ConfigMgrWebURL.Text)) -and (![string]::IsNullOrEmpty($SecretKey.Text))) {
			global:Write-CMLogEntry -Value "======== ConfigMgr WebService Diagnostics Running ========" -Severity 1
			Test-ConfigMgrWebSVC
		}
		else {
			global:Write-CMLogEntry -Value "======== ConfigMgr WebService Diagnostics Error ========" -Severity 3
			ErrorOutput -Message "Error: Please ensure you enter the ConfigMgr WebService URL and the required Secret Key value." -Severity 3
		}
	}
	...
	$FindModel_Click = {
		if (([string]$ModelSearchText.Text::IsNullOrEmpty) -ne $true) {
			# Highlight search results for Models
			[int]$ModelSearchResults = 0
			$MakeModelDataGrid.ClearSelection()
			global:Write-CMLogEntry -Value "======== Searching For Model : $($ModelSearchText.Text) ========" -Severity 1
			for ($Row = 0; $Row -lt $MakeModelDataGrid.RowCount; $Row++) {
				if ($MakeModelDataGrid.Rows[$Row].Cells[2].Value -match $ModelSearchText.Text) {
					global:Write-CMLogEntry -Value "Model found: $($MakeModelDataGrid.Rows[$Row].Cells[2].Value)" -Severity 1
					$MakeModelDataGrid.Rows[$Row].Selected = $true
			...
			global:Write-CMLogEntry -Value "Info: Please enter text to search for" -Severity 2
		}
	}
	...
	$FindDriver_Click = {
		if (([string]$HPSearchText.Text::IsNullOrEmpty) -ne $true) {
			...
			global:Write-CMLogEntry -Value "======== Searching For HP Driver : $($HPSearchText.Text) ========" -Severity 1
			for ($Row = 0; $Row -lt $HPSoftpaqDataGrid.RowCount; $Row++) {
				if ($HPSoftpaqDataGrid.Rows[$Row].Cells[1].Value -match $HPSearchText.Text) {
					global:Write-CMLogEntry -Value "Driver found: $($HPSoftpaqDataGrid.Rows[$Row].Cells[1].Value)" -Severity 1
	...
	$CustomExtractButton_Click = {
		global:Write-CMLogEntry -Value "======== Extracting Local System Drivers ========" -Severity 1
		...
		try {
			global:Write-CMLogEntry -Value "Info: Exporting local system drivers to $ExtractDriverDir" -Severity 1
			if ([boolean](Get-Command Export-WindowsDriver) -eq $false) {
				global:Write-CMLogEntry -Value "Info: Using Export-WindowsDriver cmdlet to export system drivers" -Severity 1
				...
				global:Write-CMLogEntry -Value "Info: Using DISM to export system drivers" -Severity 1
			...
			global:Write-CMLogEntry -Value "Info: Creating XML import file" -Severity 1
			...
			sleep -Seconds 3
			global:Write-CMLogEntry -Value "Info: Finished export" -Severity 1
		...
		Catch [System.Exception]{
			global:Write-CMLogEntry -Value "$($_.Exception.Message)" -Severity 2
		}
	}
	...
	$QuerySystemButton_Click = {
		$CustomPkgDataGrid.Rows.Clear()
		global:Write-CMLogEntry -Value "======== Querying Local System ========" -Severity 1
		...
		global:Write-CMLogEntry -Value "$($Product): Model detected $($CurrentModel.Make) $($CurrentModel.Model)" -Severity 1
		global:Write-CMLogEntry -Value "$($Product): Operating system detected $OSName $OSArchitecture" -Severity 1
	...
	$ShowHidePlatformButton_Click={
		if ($DriverAppTab.Location -ne "0,0") {
			$DriverAppTabLocationX = ($DriverAppTab.Location.X)
			$DriverAppTabLocationY = ($DriverAppTab.Location.Y)
			$DriverAppTabHeight = $($DriverAppTab.Height)
			$DriverAppTabWidth = $($DriverAppTab.Width)
			$DriverAppTab.Location = "0, 0"
			$DriverAppTab.Size = "1136, 411"
		}
		else {
			$DriverAppTab.Location = "4, 147"
			$DriverAppTab.Size = "1136, 264"
		}	
	}
	...
	$HideCommonSettings_CheckedChanged = {
		AdminControl -TabValue "SettingsTab" -CheckedValue $HideCommonSettings.Checked
	}
	
	$HideConfigPkgMgmt_CheckedChanged={
		AdminControl -TabValue "ConfigMgrDriverTab" -CheckedValue $HideConfigPkgMgmt.Checked
	}
	
	$HideWebService_CheckedChanged={
		AdminControl -TabValue "ConfigMgrWebSVCVisible" -CheckedValue $HideWebService.Checked
	}
	
	$HideCustomCreation_CheckedChanged = {
		AdminControl -TabValue "CustPkgTab" -CheckedValue $HideCustomCreation.Checked
	}
	
	$HideMDT_CheckedChanged = {
		AdminControl -TabValue "MDTSettingsVisible" -CheckedValue $HideMDT.Checked
	}
	...
	$Form_Cleanup_FormClosed=
	{
		...
		try
		{
			$ShowHidePlatformButton.remove_Click($ShowHidePlatformButton_Click)
			...
			$DPGridView.remove_KeyPress($DPGridView_KeyPress)
			$DPGGridView.remove_CurrentCellDirtyStateChanged($DPGGridView_CurrentCellDirtyStateChanged)
			$DPGGridView.remove_KeyPress($DPGGridView_KeyPress)
			...
			$PackageRoot.remove_CheckedChanged($PackageRoot_CheckedChanged)
			...
			$HideCommonSettings.remove_CheckedChanged($HideCommonSettings_CheckedChanged)
	...
	$DriverCleanup.SuspendLayout()
	$ProxyOptionsTab.SuspendLayout()
	$SchedulingGroupBox.SuspendLayout()
	$ProxyGroupBox.SuspendLayout()
	$AdminOptionsTab.SuspendLayout()
...
wAAAAP/////////////////////////////////////AAAAA')
	...
	$MainForm.Text = 'Driver Automation Tool: Version 6.1.5'
	...
	$LogoPanel.AutoSizeMode = 'GrowAndShrink'
	$LogoPanel.BackColor = 'White'
	$LogoPanel.Location = '-2, -1'
	$LogoPanel.Name = 'LogoPanel'
	$LogoPanel.Size = '1172, 122'
	...
	$AutomationLabel.Location = '683, 21'
...
ooA//9k=')
	...
	$DescriptionText.Location = '580, 53'
	$DescriptionText.Multiline = $True
	$DescriptionText.Name = 'DescriptionText'
	$DescriptionText.ReadOnly = $True
	$DescriptionText.Size = '563, 37'
	$DescriptionText.TabIndex = 41
	$DescriptionText.Text = 'Automates the process of downloading, extracting and importing drivers and BIOS updates into ConfigMgr, MDT and other OS deployment solutions'
	$DescriptionText.TextAlign = 'Right'
	#
	# SelectionTabs
	#
	$SelectionTabs.Controls.Add($MakeModelTab)
	$SelectionTabs.Controls.Add($SettingsTab)
	...
	$SelectionTabs.Size = '1144, 522'
	$SelectionTabs.TabIndex = 39
	#
	# MakeModelTab
	#
	$MakeModelTab.Controls.Add($ShowHidePlatformButton)
	...
	#
	# ShowHidePlatformButton
	#
	$ShowHidePlatformButton.BackColor = 'DimGray'
	$ShowHidePlatformButton.FlatStyle = 'Flat'
	$ShowHidePlatformButton.ForeColor = 'White'
	$ShowHidePlatformButton.Location = '945, 59'
	$ShowHidePlatformButton.Name = 'ShowHidePlatformButton'
	$ShowHidePlatformButton.Size = '188, 23'
	$ShowHidePlatformButton.TabIndex = 106
	$ShowHidePlatformButton.Text = 'Show / Hide Platform'
	$ShowHidePlatformButton.UseCompatibleTextRendering = $True
	$ShowHidePlatformButton.UseVisualStyleBackColor = $False
	$ShowHidePlatformButton.add_Click($ShowHidePlatformButton_Click)
	#
	# MakeModelIcon
	#
	#region Binary Data
...
W/MmV9clXL8AAAAASUVORK5CYII=')
	...
	$PlatformPanel.Controls.Add($groupbox3)
	$PlatformPanel.Anchor = 'Top, Bottom, Left'
	$PlatformPanel.BackColor = 'WhiteSmoke'
	$PlatformPanel.Location = '0, 83'
	$PlatformPanel.Name = 'PlatformPanel'
	$PlatformPanel.Size = '1136, 411'
	...
	$ModelDriverTab.Size = '1120, 236'
	$ModelDriverTab.TabIndex = 0
	$ModelDriverTab.Text = 'Model Selection'
	#
	# SelectAll
	#
	$SelectAll.Anchor = 'Top, Left, Right'
	$SelectAll.BackColor = 'Gray'
	$SelectAll.FlatStyle = 'Popup'
	$SelectAll.ForeColor = 'White'
	$SelectAll.Location = '864, 6'
	$SelectAll.Name = 'SelectAll'
	$SelectAll.Size = '122, 23'
	$SelectAll.TabIndex = 99
	...
	# XMLLoadingLabel
	#
	$XMLLoadingLabel.Anchor = 'Top, Bottom, Left, Right'
	$XMLLoadingLabel.AutoSize = $True
	$XMLLoadingLabel.Font = 'Segoe UI, 9pt, style=Bold'
	$XMLLoadingLabel.ForeColor = 'White'
	$XMLLoadingLabel.Location = '106, 21'
	$XMLLoadingLabel.Name = 'XMLLoadingLabel'
	$XMLLoadingLabel.Size = '213, 21'
	...
	# ClearModelSelection
	#
	$ClearModelSelection.Anchor = 'Top, Left, Right'
	$ClearModelSelection.FlatStyle = 'Popup'
	$ClearModelSelection.Location = '992, 6'
	$ClearModelSelection.Name = 'ClearModelSelection'
	$ClearModelSelection.Size = '122, 23'
	$ClearModelSelection.TabIndex = 9
	...
	$FindModel.Location = '462, 6'
	$FindModel.Name = 'FindModel'
	$FindModel.Size = '75, 24'
	$FindModel.TabIndex = 8
	$FindModel.Text = 'Find'
	...
	$labelSearchModels.Location = '12, 9'
	...
	# ModelSearchText
	#
	$ModelSearchText.Location = '131, 7'
	$ModelSearchText.Name = 'ModelSearchText'
	$ModelSearchText.Size = '325, 23'
	$ModelSearchText.TabIndex = 6
	#
	# MakeModelDataGrid
	#
	...
	$MakeModelDataGrid.Size = '1121, 200'
	...
	$ClearHPDriverSelection.Location = '1009, 9'
	$ClearHPDriverSelection.Name = 'ClearHPDriverSelection'
	$ClearHPDriverSelection.Size = '108, 23'
	...
	$FindDriver.Location = '462, 6'
	$FindDriver.Name = 'FindDriver'
	$FindDriver.Size = '75, 24'
	...
	$labelSearchCatalogue.Location = '12, 9'
	...
	$HPSoftpaqDataGrid.Location = '0, 40'
	...
	$HPSoftpaqDataGrid.Size = '1117, 196'
	...
	$ArchitectureComboxBox.Location = '32, 106'
	$ArchitectureComboxBox.Margin = '4, 3, 4, 3'
	$ArchitectureComboxBox.Name = 'ArchitectureComboxBox'
	$ArchitectureComboxBox.Size = '204, 23'
	$ArchitectureComboxBox.TabIndex = 56
	#
	# OSComboBox
	#
	...
	$OSComboBox.Location = '32, 51'
	$OSComboBox.Margin = '4, 3, 4, 3'
	$OSComboBox.Name = 'OSComboBox'
	$OSComboBox.Size = '204, 23'
	$OSComboBox.TabIndex = 55
	...
	$ArchitectureCheckBox.Location = '32, 82'
	...
	$OperatingSysLabel.Location = '32, 28'
	...
	$DeploymentGroupBox.Size = '403, 143'
	...
	$SelectDeployLabel.Location = '32, 27'
	...
	$DownloadTypeLabel.Location = '32, 82'
	...
	# groupbox3
	#
	$groupbox3.Controls.Add($FindModelsButton)
	$groupbox3.Controls.Add($MicrosoftCheckBox)
	$groupbox3.Controls.Add($HPCheckBox)
	$groupbox3.Controls.Add($LenovoCheckBox)
	$groupbox3.Controls.Add($DellCheckBox)
	$groupbox3.Anchor = 'Top, Right'
	$groupbox3.Font = 'Segoe UI, 9.75pt, style=Bold'
	$groupbox3.ForeColor = 'Black'
	$groupbox3.Location = '706, 5'
	$groupbox3.Name = 'groupbox3'
	$groupbox3.Size = '424, 143'
	$groupbox3.TabIndex = 68
	$groupbox3.TabStop = $False
	$groupbox3.Text = 'Manufacturer Selection'
	...
	$FindModelsButton.Location = '194, 56'
	$FindModelsButton.Margin = '4, 3, 4, 3'
	$FindModelsButton.Name = 'FindModelsButton'
	$FindModelsButton.Size = '189, 47'
	$FindModelsButton.TabIndex = 76
	...
	$HPCheckBox.Location = '18, 81'
	$HPCheckBox.Name = 'HPCheckBox'
	$HPCheckBox.Size = '159, 24'
	$HPCheckBox.TabIndex = 86
	...
	$LenovoCheckBox.Location = '18, 53'
	$LenovoCheckBox.Name = 'LenovoCheckBox'
	$LenovoCheckBox.Size = '93, 24'
	$LenovoCheckBox.TabIndex = 87
	...
	# SettingsTab
	#
	$SettingsTab.Controls.Add($SettingsIcon)
	$SettingsTab.Controls.Add($labelSettingsStorageConfi)
	$SettingsTab.Controls.Add($SettingsTabs)
	$SettingsTab.Controls.Add($SettingsPanel)
	$SettingsTab.BackColor = 'Gray'
	$SettingsTab.Location = '4, 24'
	$SettingsTab.Name = 'SettingsTab'
	$SettingsTab.Size = '1136, 494'
	$SettingsTab.TabIndex = 7
	$SettingsTab.Text = 'Common Settings'
	#
	# SettingsIcon
	#
	#region Binary Data
...
5e4AAAAASUVORK5CYII=')
	...
	#
	# labelSettingsStorageConfi
	#
	$labelSettingsStorageConfi.AutoSize = $True
	$labelSettingsStorageConfi.Font = 'Segoe UI Semibold, 16pt, style=Bold'
	$labelSettingsStorageConfi.ForeColor = 'White'
	$labelSettingsStorageConfi.Location = '90, 24'
	$labelSettingsStorageConfi.Name = 'labelSettingsStorageConfi'
	$labelSettingsStorageConfi.Size = '384, 35'
	$labelSettingsStorageConfi.TabIndex = 101
	$labelSettingsStorageConfi.Text = 'Settings | Storage, ConfigMgr + More'
	$labelSettingsStorageConfi.UseCompatibleTextRendering = $True
	#
	# SettingsTabs
	#
	$SettingsTabs.Controls.Add($MainOptionsTab)
	$SettingsTabs.Controls.Add($ConfigMgrDPOptionsTab)
	$SettingsTabs.Controls.Add($PackageOptionsTab)
	$SettingsTabs.Controls.Add($ProxyOptionsTab)
	$SettingsTabs.Controls.Add($AdminOptionsTab)
	$SettingsTabs.Anchor = 'Top, Bottom, Left, Right'
	$SettingsTabs.Appearance = 'FlatButtons'
	$SettingsTabs.Location = '4, 83'
	$SettingsTabs.Margin = '4, 4, 4, 4'
	$SettingsTabs.Name = 'SettingsTabs'
	$SettingsTabs.SelectedIndex = 0
	$SettingsTabs.Size = '1128, 414'
	$SettingsTabs.SizeMode = 'FillToRight'
	$SettingsTabs.TabIndex = 84
	#
	# MainOptionsTab
	#
	$MainOptionsTab.Controls.Add($groupbox1)
	$MainOptionsTab.Controls.Add($StoageGroupBox)
	$MainOptionsTab.BackColor = 'WhiteSmoke'
	$MainOptionsTab.Location = '4, 27'
	$MainOptionsTab.Name = 'MainOptionsTab'
	$MainOptionsTab.Size = '1120, 383'
	$MainOptionsTab.TabIndex = 4
	$MainOptionsTab.Text = 'ConfigMgr Site Server / Storage Locations'
	#
	# groupbox1
	#
	...
	$groupbox1.Location = '3, 20'
	$groupbox1.Name = 'groupbox1'
	$groupbox1.Size = '554, 349'
	...
	$ConfigMgrImport.Location = '173, 205'
	...
	$labelSelectKnownModels.Font = 'Segoe UI Semibold, 9pt'
	$labelSelectKnownModels.ForeColor = 'Black'
	$labelSelectKnownModels.Location = '31, 208'
	...
	#
	$ConifgSiteInstruction.Anchor = 'Bottom, Left'
	...
	$SiteServerInput.Location = '173, 127'
	...
	$SiteServerLabel.Location = '89, 133'
	...
	$StoageGroupBox.Controls.Add($PackageBrowseButton)
	$StoageGroupBox.Controls.Add($DownloadBrowseButton)
	$StoageGroupBox.Controls.Add($PackagePathLabel)
	$StoageGroupBox.Controls.Add($PackagePathTextBox)
	$StoageGroupBox.Controls.Add($DownloadPathTextBox)
	$StoageGroupBox.Font = 'Segoe UI, 9.75pt, style=Bold'
	$StoageGroupBox.Location = '564, 14'
	$StoageGroupBox.Name = 'StoageGroupBox'
	$StoageGroupBox.Size = '553, 355'
	$StoageGroupBox.TabIndex = 84
	...
	#
	$StoragePathInstruction.Anchor = 'Bottom, Left'
	...
	$StoragePathInstruction.Location = '8, 33'
	...
	$StoragePathInstruction.Size = '539, 58'
	...
	$DownloadBrowseButton.Location = '39, 168'
	...
	$PackagePathLabel.Location = '41, 212'
	$PackagePathLabel.Margin = '4, 0, 4, 0'
	$PackagePathLabel.Name = 'PackagePathLabel'
	$PackagePathLabel.Size = '82, 21'
	$PackagePathLabel.TabIndex = 83
	...
	$PackagePathTextBox.Location = '41, 241'
	$PackagePathTextBox.Margin = '4, 4, 4, 4'
	$PackagePathTextBox.Name = 'PackagePathTextBox'
	$PackagePathTextBox.Size = '431, 27'
	$PackagePathTextBox.TabIndex = 81
	...
	$DownloadPathTextBox.Location = '39, 133'
	...
	#
	$ConfigMgrDPOptionsTab.Controls.Add($DPGroupBox)
	$ConfigMgrDPOptionsTab.BackColor = 'WhiteSmoke'
	$ConfigMgrDPOptionsTab.Location = '4, 27'
	$ConfigMgrDPOptionsTab.Name = 'ConfigMgrDPOptionsTab'
	$ConfigMgrDPOptionsTab.Size = '1120, 383'
	$ConfigMgrDPOptionsTab.TabIndex = 5
	$ConfigMgrDPOptionsTab.Text = 'ConfigMgr Package Distribution'
	#
	# DPGroupBox
	#
	...
	$DPGroupBox.Location = '12, 27'
	$DPGroupBox.Name = 'DPGroupBox'
	$DPGroupBox.Size = '1093, 304'
	$DPGroupBox.TabIndex = 84
	...
	$EnableBinaryDifCheckBox.Location = '28, 246'
	$EnableBinaryDifCheckBox.Name = 'EnableBinaryDifCheckBox'
	$EnableBinaryDifCheckBox.Size = '245, 24'
	...
	$PriorityLabel.Location = '300, 250'
	...
	$DistributionPriorityCombo.Location = '372, 247'
	$DistributionPriorityCombo.Name = 'DistributionPriorityCombo'
	$DistributionPriorityCombo.Size = '152, 23'
	...
	$DPSelectionsTabs.Anchor = 'Top, Left, Right'
	...
	$DPSelectionsTabs.Size = '1054, 201'
	...
	$FallbackPkgGroup.Location = '4, 282'
	$FallbackPkgGroup.Name = 'FallbackPkgGroup'
	$FallbackPkgGroup.Size = '1114, 95'
	...
	$FallbackArcCombo.Location = '587, 59'
	...
	$FallbackOSCombo.Location = '587, 16'
	...
	$ArchitectureLabel.Location = '477, 62'
	...
	$lOperatingSystemLabel.Location = '471, 19'
	...
	$PackageCreation.Location = '565, 6'
	$PackageCreation.Name = 'PackageCreation'
	$PackageCreation.Size = '549, 270'
	$PackageCreation.TabIndex = 109
	$PackageCreation.TabStop = $False
	$PackageCreation.Text = 'Package Creation'
	...
	$CustPackageDest.Location = '47, 137'
	...
	$SpecifyCustomPath.Location = '36, 105'
	...
	$PackageRoot.add_CheckedChanged($PackageRoot_CheckedChanged)
	#
	# DriverCleanup
	#
	$DriverCleanup.Controls.Add($RemoveBIOSText)
	$DriverCleanup.Controls.Add($RemoveLegacyBIOSCheckbox)
	$DriverCleanup.Controls.Add($CleanUpText)
	$DriverCleanup.Controls.Add($CleanSourceText)
	$DriverCleanup.Controls.Add($CleanUnusedCheckBox)
	$DriverCleanup.Controls.Add($RemoveSuperText)
	$DriverCleanup.Controls.Add($RemoveLegacyDriverCheckbox)
	$DriverCleanup.Controls.Add($RemoveDriverSourceCheckbox)
	$DriverCleanup.Font = 'Segoe UI, 9.75pt, style=Bold'
	$DriverCleanup.ForeColor = 'Black'
	$DriverCleanup.Location = '6, 6'
	$DriverCleanup.Name = 'DriverCleanup'
	$DriverCleanup.Size = '551, 270'
	$DriverCleanup.TabIndex = 108
	$DriverCleanup.TabStop = $False
	$DriverCleanup.Text = 'Driver Cleanup'
	$DriverCleanup.UseCompatibleTextRendering = $True
	#
	# RemoveBIOSText
	#
	...
	$RemoveLegacyBIOSCheckbox.Location = '26, 144'
	$RemoveLegacyBIOSCheckbox.Name = 'RemoveLegacyBIOSCheckbox'
	$RemoveLegacyBIOSCheckbox.Size = '396, 24'
	$RemoveLegacyBIOSCheckbox.TabIndex = 108
	...
	# ProxyOptionsTab
	#
	$ProxyOptionsTab.Controls.Add($SchedulingGroupBox)
	$ProxyOptionsTab.Controls.Add($ProxyGroupBox)
	$ProxyOptionsTab.BackColor = 'WhiteSmoke'
	$ProxyOptionsTab.Location = '4, 27'
	$ProxyOptionsTab.Margin = '4, 4, 4, 4'
	$ProxyOptionsTab.Name = 'ProxyOptionsTab'
	$ProxyOptionsTab.Padding = '3, 3, 3, 3'
	$ProxyOptionsTab.Size = '1120, 383'
	$ProxyOptionsTab.TabIndex = 1
	$ProxyOptionsTab.Text = 'Proxy Server / Scheduling'
	#
	# SchedulingGroupBox
	#
	...
	$SchedulingGroupBox.Location = '563, 6'
	$SchedulingGroupBox.Name = 'SchedulingGroupBox'
	$SchedulingGroupBox.Size = '553, 321'
	$SchedulingGroupBox.TabIndex = 105
	...
	$SchedulingInstruction.Size = '515, 46'
	...
	#
	$ScriptDirectoryBrowseButton.Anchor = 'Left'
	$ScriptDirectoryBrowseButton.Location = '421, 164'
	$ScriptDirectoryBrowseButton.Margin = '4, 4, 4, 4'
	$ScriptDirectoryBrowseButton.Name = 'ScriptDirectoryBrowseButton'
	$ScriptDirectoryBrowseButton.Size = '45, 26'
	...
	# UsernameTextBox
	#
	$UsernameTextBox.Anchor = 'Left'
	$UsernameTextBox.BackColor = 'White'
	$UsernameTextBox.Font = 'Segoe UI Semibold, 9pt, style=Bold'
	$UsernameTextBox.Location = '199, 195'
	...
	$ScheduleJobButton.Location = '199, 263'
	$ScheduleJobButton.Name = 'ScheduleJobButton'
	$ScheduleJobButton.Size = '216, 34'
	...
	# PasswordTextBox
	#
	$PasswordTextBox.Anchor = 'Left'
	$PasswordTextBox.BackColor = 'White'
	$PasswordTextBox.Font = 'Segoe UI Semibold, 9pt, style=Bold'
	$PasswordTextBox.Location = '199, 226'
	...
	$ScriptLocation.Location = '199, 166'
	...
	$ProxyGroupBox.Location = '12, 6'
	$ProxyGroupBox.Name = 'ProxyGroupBox'
	$ProxyGroupBox.Size = '545, 322'
	$ProxyGroupBox.TabIndex = 104
...
To set your proxy specify the server and port number along with a username and password. Proxy authentication and other settings can also be set inside the script.'
	...
	$labelProxyServer.Location = '22, 176'
	...
	$ProxyPswdInput.Location = '136, 249'
	...
	$ProxyServerInput.Location = '136, 171'
	...
	$ProxyUserInput.Location = '136, 211'
	...
	$UseProxyServerCheckbox.Location = '22, 133'
	...
	# AdminOptionsTab
	#
	$AdminOptionsTab.Controls.Add($AdminControlsInstruction)
	$AdminOptionsTab.Controls.Add($groupbox4)
	$AdminOptionsTab.BackColor = 'WhiteSmoke'
	$AdminOptionsTab.Location = '4, 27'
	$AdminOptionsTab.Name = 'AdminOptionsTab'
	$AdminOptionsTab.Padding = '3, 3, 3, 3'
	$AdminOptionsTab.Size = '1120, 383'
	$AdminOptionsTab.TabIndex = 6
	$AdminOptionsTab.Text = 'Admin Controls'
	#
	...
	$AdminControlsInstruction.Location = '12, 19'
	...
	$AdminControlsInstruction.TabIndex = 63
	$AdminControlsInstruction.TabStop = $False
	$AdminControlsInstruction.Text = 'Here you can opt to hide individual tabs or lock controls via registry settigns'
	#
	# groupbox4
	#
	$groupbox4.Controls.Add($TabControlGroup)
	$groupbox4.Font = 'Segoe UI, 9.75pt, style=Bold'
	$groupbox4.Location = '6, 60'
	$groupbox4.Name = 'groupbox4'
	$groupbox4.Size = '1108, 317'
	...
	$HideCommonSettings.add_CheckedChanged($HideCommonSettings_CheckedChanged)
	#
	# HideCustomCreation
	#
	$HideCustomCreation.Font = 'Segoe UI Semibold, 9pt'
	$HideCustomCreation.ForeColor = 'Black'
	$HideCustomCreation.Location = '25, 180'
	...
	$HideConfigPkgMgmt.Location = '25, 120'
	...
	$HideWebService.Location = '25, 150'
...
5e4AAAAASUVORK5CYII=')
	...
	$DeploymentShareGrid.Location = '0, 315'
	...
	$DeploymentShareGrid.Size = '1136, 179'
	...
	$MDTSettingsPanel.Size = '1140, 404'
	...
	$FolderStructureGroup.Size = '399, 192'
	...
	$TotalControlExampleLabel.Size = '383, 40'
...
Structure: Lenovo\T460S\Windows 10 x64\A08\"
	...
	$FolderStructureLabel.Size = '144, 20'
...
'
	...
	$ScriptLocationLabel.Size = '106, 20'
...
'
	...
	$MDTScriptBrowseButton.Size = '45, 26'
	...
	$ConfigMgrDriverTab.Controls.Add($SelectNoneButton)
	$ConfigMgrDriverTab.Controls.Add($SelectAllButton)
	...
	$ConfigMgrDriverTab.Size = '1136, 494'
...
YII=')
	...
	$PackageUpdatePanel.Size = '417, 100'
	...
	$PackageUpdateNotice.Location = '132, 34'
	...
	$SelectNoneButton.Location = '207, 449'
	...
	$SelectAllButton.Location = '12, 449'
	...
	$DeploymentStateCombo.Location = '572, 24'
	...
	$DeploymentStateLabel.Location = '407, 27'
	...
	$PackageTypeCombo.Location = '146, 24'
	...
	[void]$ConfigMgrPkgActionCombo.Items.Add('Move to Windows 10 1611')
	$ConfigMgrPkgActionCombo.Location = '755, 374'
...
MQAAAABJRU5ErkJggg==')
	...
	$WebServiceDataGrid.Location = '287, 259'
	...
	$WebServiceDataGrid.Size = '852, 239'
	...
	$ConfigMgrWebSvcLabel.Location = '24, 152'
	...
	$WebServiceVersion.Location = '209, 191'
...
Enter the ConfigMgr web service URL and secret key, then click on the "Connect ConfigMgr Web Service" button. The results are displayed in the below section.'
	...
	$WebServiceVersionLabel.Location = '24, 191'
	...
	$WebServiceStatusDescription.Location = '209, 251'
	...
	$ConfigMgrWebServuceULabel.Location = '622, 26'
	$ConfigMgrWebServuceULabel.Margin = '4, 0, 4, 0'
	$ConfigMgrWebServuceULabel.Name = 'ConfigMgrWebServuceULabel'
	$ConfigMgrWebServuceULabel.Size = '169, 21'
	$ConfigMgrWebServuceULabel.TabIndex = 62
	$ConfigMgrWebServuceULabel.Text = 'ConfigMgr Web Service URL'
	...
	$StatusDescriptionLabel.Location = '24, 251'
	...
	$SecretKeyLabel.Location = '726, 66'
	...
	$StatusCodeLabel.Location = '24, 221'
	...
	$BIOSPackageCount.Location = '209, 341'
	...
	$WebServiceResponseTime.Location = '209, 281'
	...
	$ResponseTimeLabel.Location = '24, 281'
	...
	$DriverPackageCount.Location = '209, 311'
	...
	$BIOSPackageCountLabel.Location = '24, 341'
	...
	$WebServiceStatusCode.Location = '209, 221'
	...
	$DriverPackageCountLabel.Location = '24, 311'
	...
	$PkgImporting.Size = '417, 94'
	$PkgImporting.TabIndex = 100
	$PkgImporting.Visible = $False
	#
	# PkgImportingText
	#
	$PkgImportingText.Dock = 'Fill'
...
g3Vkg/XT6vsDwJaHNLYf3WUAAAAASUVORK5CYII=')
	...
	$System_Windows_Forms_DataGridViewCellStyle_5.BackColor = 'White'
	$System_Windows_Forms_DataGridViewCellStyle_5.Font = 'Segoe UI Semibold, 9pt, style=Bold'
	...
	$CustomPkgDataGrid.ColumnHeadersDefaultCellStyle = $System_Windows_Forms_DataGridViewCellStyle_5
	...
	$System_Windows_Forms_DataGridViewCellStyle_6.BackColor = 'White'
	$System_Windows_Forms_DataGridViewCellStyle_6.Font = 'Segoe UI Semibold, 9pt, style=Bold'
	...
	$CustomPkgDataGrid.DefaultCellStyle = $System_Windows_Forms_DataGridViewCellStyle_6
	$CustomPkgDataGrid.GridColor = 'White'
	$CustomPkgDataGrid.Location = '0, 163'
	...
	$System_Windows_Forms_DataGridViewCellStyle_7.BackColor = 'White'
	$System_Windows_Forms_DataGridViewCellStyle_7.Font = 'Segoe UI Semibold, 9pt, style=Bold'
	$System_Windows_Forms_DataGridViewCellStyle_7.ForeColor = 'Black'
	$System_Windows_Forms_DataGridViewCellStyle_7.SelectionBackColor = 'Maroon'
	...
	$CustomPkgDataGrid.RowHeadersDefaultCellStyle = $System_Windows_Forms_DataGridViewCellStyle_7
	...
	$CustomPkgDataGrid.Size = '1133, 248'
	...
	$CustomDeploymentLabel.Location = '18, 31'
	...
	$CustomPkgPlatform.Location = '155, 28'
	...
	$groupbox2.Location = '7, 334'
	$groupbox2.Name = 'groupbox2'
	$groupbox2.Size = '1130, 74'
	...
	$LogTab.Controls.Add($ModelProgressOverlay)
	$LogTab.Controls.Add($ModelProgressLabel)
	$LogTab.Controls.Add($ProgressBar)
	$LogTab.Controls.Add($LogPanel)
	$LogTab.BackColor = 'Gray'
	$LogTab.Location = '4, 24'
	$LogTab.Name = 'LogTab'
	$LogTab.Size = '1136, 494'
	$LogTab.TabIndex = 8
...
RU5ErkJggg==')
	...
	$ModelProgressOverlay.Location = '665, 29'
	$ModelProgressOverlay.Margin = '4, 3, 4, 3'
	$ModelProgressOverlay.Name = 'ModelProgressOverlay'
	$ModelProgressOverlay.Size = '458, 30'
	$ModelProgressOverlay.Style = 'Continuous'
	$ModelProgressOverlay.TabIndex = 30
	#
	# ModelProgressLabel
	#
	$ModelProgressLabel.Anchor = 'Top, Bottom, Left, Right'
	$ModelProgressLabel.AutoSize = $True
	$ModelProgressLabel.BackColor = 'Transparent'
	$ModelProgressLabel.Font = 'Segoe UI, 9.75pt, style=Bold'
	$ModelProgressLabel.ForeColor = 'White'
	$ModelProgressLabel.Location = '562, 34'
	$ModelProgressLabel.Margin = '4, 0, 4, 0'
	$ModelProgressLabel.Name = 'ModelProgressLabel'
	$ModelProgressLabel.Size = '85, 22'
	$ModelProgressLabel.TabIndex = 32
	$ModelProgressLabel.Text = 'Job Progress'
	$ModelProgressLabel.UseCompatibleTextRendering = $True
	#
	# ProgressBar
	#
	$ProgressBar.Anchor = 'Top, Left, Right'
	$ProgressBar.Location = '665, 29'
	$ProgressBar.Margin = '4, 3, 4, 3'
	$ProgressBar.Name = 'ProgressBar'
	$ProgressBar.Size = '459, 30'
	...
	$LogPanel.BackColor = 'WhiteSmoke'
	$LogPanel.Location = '0, 83'
	$LogPanel.Name = 'LogPanel'
	$LogPanel.Size = '1140, 411'
	$LogPanel.TabIndex = 72
	#
	# ProgressListBox
	#
	$ProgressListBox.Anchor = 'Top, Bottom, Left, Right'
	...
	$AboutPanelRight.Anchor = 'Top, Bottom, Left'
	$AboutPanelRight.BackColor = 'WhiteSmoke'
	$AboutPanelRight.Location = '690, 83'
	$AboutPanelRight.Name = 'AboutPanelRight'
	$AboutPanelRight.Size = '449, 411'
	...
	$MSTechnetSiteLaunchButton.Location = '35, 350'
	$MSTechnetSiteLaunchButton.Name = 'MSTechnetSiteLaunchButton'
	$MSTechnetSiteLaunchButton.Size = '382, 30'
	$MSTechnetSiteLaunchButton.TabIndex = 2
	$MSTechnetSiteLaunchButton.Text = 'Launch MS Technet Site'
...
0g4rXGmHFa60wwpX2mGFK+2wwpV2WOFKO6xwpRV7B/4LyowiO2CXutoAAAAASUVORK5CYII=')
	...
	$ModernDriverDesc.Location = '14, 301'
	$ModernDriverDesc.Name = 'ModernDriverDesc'
	$ModernDriverDesc.Size = '640, 43'
...
'
	...
	$AboutToolDesc.Size = '641, 194'
	$AboutToolDesc.TabIndex = 62
	$AboutToolDesc.Text = 'LEGAL INFORMATION:
...
'
	...
	$MDMLaunchButton.Location = '14, 350'
	$MDMLaunchButton.Name = 'MDMLaunchButton'
	$MDMLaunchButton.Size = '641, 30'
...
'
	...
	# StartDownloadButton
	#
	$StartDownloadButton.Anchor = 'Bottom, Left'
	...
	$StartDownloadButton.Location = '589, 655'
	...
	$System_Windows_Forms_DataGridViewCellStyle_8.ForeColor = 'Black'
	$Platform.DefaultCellStyle = $System_Windows_Forms_DataGridViewCellStyle_8
	...
	$OperatingSystem.DefaultCellStyle = $System_Windows_Forms_DataGridViewCellStyle_8
	...
	$Architecture.DefaultCellStyle = $System_Windows_Forms_DataGridViewCellStyle_8
	...
	$System_Windows_Forms_DataGridViewCellStyle_9.Alignment = 'MiddleCenter'
	$System_Windows_Forms_DataGridViewCellStyle_9.BackColor = '224, 224, 224'
	$System_Windows_Forms_DataGridViewCellStyle_9.Font = 'Segoe UI, 9pt, style=Bold'
	$System_Windows_Forms_DataGridViewCellStyle_9.ForeColor = 'Black'
	$Browse.DefaultCellStyle = $System_Windows_Forms_DataGridViewCellStyle_9
	...
	$DPName.Name = 'DPName'
	$DPName.ReadOnly = $True
	$DPName.Resizable = 'True'
	$DPName.SortMode = 'NotSortable'
	#
	# ModelSelected
	#
	...
	$AdminOptionsTab.ResumeLayout()
	$ProxyGroupBox.ResumeLayout()
	$SchedulingGroupBox.ResumeLayout()
	$ProxyOptionsTab.ResumeLayout()
	$DriverCleanup.ResumeLayout()
	...
	$MainOptionsTab.ResumeLayout()
	$SettingsTabs.ResumeLayout()
	$SettingsTab.ResumeLayout()
...
#region Source: Globals.ps1
	...
	$Dell64BIOSUtil = "https://downloads.dell.com/FOLDER04165397M/1/Flash64W.zip"
	# Define Dell Download Sources
	...
	function ErrorOutput {
		...
		global:Write-CMLogEntry -Value "======== Errors(s) Occurred ========" -Severity $Severity
		global:Write-CMLogEntry -Value $Message -Severity $Severity
		global:Write-CMLogEntry -Value " " -Severity $Severity
		$ProgressListBox.ForeColor = "Maroon"
		$SelectionTabs.SelectedTab = $LogTab
	}
	...
	function SCCMPkgFolder {
		...
		if ($PackageRoot.Checked -eq $true) {
			...
			global:Write-CMLogEntry -Value "Info: Using SCCM console packages root folder for packages " -Severity 1
		}
		elseif (($SpecifyCustomPath.Checked -eq $true) -and ((Test-Path -Path $CustPackageDest.text) -eq $false)) {
			...
			While ($CustomPathDirCount -ne 0) {
				if (![string]::IsNullOrEmpty($CustPackageDest.text)) {
					if ($CustomPathDirStep -ne 0) {
						...
						global:Write-CMLogEntry -Value "Info: Creating custom package subfolder - $CustPackagePath " -Severity 1
					}
					elseif ($CustomPathDirStep -eq 0) {
						$CustPackagePath = $CustPackageDest.text.split("\")[$CustomPathDirStep] + "\"
						global:Write-CMLogEntry -Value "Info: Creating custom package root folder - $CustPackagePath " -Severity 1
					}
			...
			global:Write-CMLogEntry -Value "Info: Using SCCM console BIOS package folder - $global:VendorBIOSFolder" -Severity 1
			global:Write-CMLogEntry -Value "Info: Using SCCM console Driver package folder - $global:VendorDriverFolder" -Severity 1
		}
	}
	...
	function QuerySiteCode ($SiteServer) {
		try {
			...
			ErrorOutput -Message "Error: $($_.Exception.Message)" -Severity 3
			$SiteCodeError = $true
		}
		if (($SiteCodeObjects -ne $null) -and ($SiteCodeError -ne $true)) {
			foreach ($SiteCodeObject in $SiteCodeObjects) {
				if ($SiteCodeObject.ProviderForLocalSite -eq $true) {
					...
					global:Write-CMLogEntry -Value "Info: Site Code Found: $($global:SiteCode)" -Severity 1
	...
	function UpdateModeList {
		...
		global:Write-CMLogEntry -Value "======== Querying Model List(s) ========" -Severity 1
		# Reset Product Listbox
		$MakeModelDataGrid.ClearSelection()
		$XMLLoading.Visible = $true
		$XMLDownloadStatus.Visible = $true
		$XMLDownloadStatus.Text = $null
		...
		if ($HPCheckBox.Checked -eq $true) {
			...
			if ((Test-Path -Path $global:TempDirectory\$HPCabFile) -eq $false) {
				...
				try {
					...
					if ((Test-Path -Path $global:TempDirectory\$HPXMLFile) -eq $false) {
						...
						Expand "$global:TempDirectory\$HPCabFile" -F:* "$global:TempDirectory\$HPXMLFile" | Out-Null
					}
				}
				catch {
					ErrorOutput -Message "Error: $($_.Exception.Message)" -Severity 3
			...
			if ($global:HPModelSoftPaqs -eq $null) {
				...
				global:Write-CMLogEntry -Value "Info: Reading driver pack XML file - $global:TempDirectory\$HPXMLFile" -Severity 1
			...
			if ((Test-Path -Path $(Join-Path -Path $global:TempDirectory -ChildPath $HPSoftPaqXMLFile)) -eq $false) {
				try {
					...
					if ((Test-Path -Path $global:TempDirectory\$HPSoftPaqCabFile) -eq $false) {
						global:Write-CMLogEntry -Value "======== Downloading HP SoftPaq List ========" -Severity 1
						# Download HP Model Cabinet File
						global:Write-CMLogEntry -Value "Info: Downloading HP softpaq cabinet file from $HPSoftPaqCab" -Severity 1
						if ($global:ProxySettingsSet -eq $true) {
					...
					if ((Test-Path -Path $(Join-Path $global:TempDirectory -ChildPath $HPSoftPaqXMLFile)) -eq $false) {
						# Expand Cabinet File
						global:Write-CMLogEntry -Value "Info: Expanding HP softpaq cabinet file: $HPSoftPaqCabFile" -Severity 1
						Expand "$global:TempDirectory\$HPSoftPaqCabFile" -F:"*.XML" "$global:TempDirectory\$HPSoftPaqXMLFile" | Out-Null
			...
			if (($global:HPSoftPaqXML -eq $null) -and ((Test-Path -Path $(Join-Path $global:TempDirectory -ChildPath $HPSoftPaqXMLFile)) -eq $true)) {
				try {
					...
					global:Write-CMLogEntry -Value "Info: Reading softpaq XML file - $global:TempDirectory\$HPSoftPaqXMLFile" -Severity 1
					...
					global:Write-CMLogEntry -Value "Info: Adding $($global:HPSoftPaqList.Count) available SoftPaq downloads" -Severity 1
				...
				global:Write-CMLogEntry -Value "Warning: Failed to find HP XML file: $HPSoftPaqCabFile" -Severity 1
			}
			...
			# Add Known HP Models
			if (($SiteCode -ne "N/A" -or $SiteCode -ne $null) -and ($ConfigMgrImport.text -eq "yes")) {
					...
					foreach ($HPKnownModel in $HPKnownModels) {
						...
						if ($HPKnownModel -gt $null) {
							if ($HPKnownProducts -notcontains $HPKnownModel) {
								global:Write-CMLogEntry -Value "Info: Adding $HPKnownModel to HP known models" -Severity 1
								$HPKnownProducts.Add($HPKnownModel) | Out-Null
							}
						}
					}
					global:Write-CMLogEntry -Value "Info: Found: $(($HPKnownProducts).count) known HP models" -Severity 1
				}
			}
		...
		if ($DellCheckBox.Checked -eq $true) {
			...
			if ((Test-Path -Path $global:TempDirectory\$DellCabFile) -eq $false) {
				...
				global:Write-CMLogEntry -Value "Info: Downloading Dell product cabinet file from $DellXMLCabinetSource" -Severity 1
				...
				try {
					...
					if ((Test-Path -Path $global:TempDirectory\$DellXMLFile) -eq $false) {
						...
						Expand "$global:TempDirectory\$DellCabFile" -F:* "$global:TempDirectory\$DellXMLFile" | Out-Null
					}
				}
				catch {
					ErrorOutput -Message "Error: $($_.Exception.Message)" -Severity 3
			...
			if ($global:DellModelXML -eq $null) {
				# Read XML File
				global:Write-CMLogEntry -Value "Info: Reading driver pack XML file - $global:TempDirectory\$DellXMLFile" -Severity 1
				...
				if ($DellKnownModels -ne $null) {
					foreach ($DellKnownModel in $DellKnownModels.Model) {
						if ($DellKnownProducts -notcontains $DellKnownModel) {
							...
							global:Write-CMLogEntry -Value "Info: Adding $DellKnownModel to Dell known models" -Severity 1
						}
					}
					global:Write-CMLogEntry -Value "Info: Found: $(($DellKnownProducts).count) known Dell models" -Severity 1
				}
			}
			if (($DellModels).Count -gt "0") {
				global:Write-CMLogEntry -Value "Info: Found $(($DellModels).count) Dell model driver packs for $($OSComboBox.text) $($ArchitectureComboxBox.text)" -Severity 1
			}
			else {
				global:Write-CMLogEntry -Value "Info: No Dell models found. If you are using a proxy server please specify the proxy in the Proxy Server Settings tab" -Severity 2
			}
		}
			...
			if (($SiteCode -ne "N/A" -or $SiteCode -ne $null) -and ($ConfigMgrImport.text -eq "yes")) {
				...
				if ($LenovoKnownModels -ne $null) {
					foreach ($LenovoKnownModel in $LenovoKnownModels.Model) {
						$LenovoKnownModel = $(LenovoModelTypeFinder -ModelType $($LenovoKnownModel.Substring(0, 4)))
						If ($LenovoKnownModel -gt $null) {
						...
						if (($LenovoKnownProducts -notcontains $LenovoKnownModel) -and (([string]::IsNullOrEmpty($LenovoKnownModel)) -ne $true)) {
							...
							global:Write-CMLogEntry -Value "Info: Adding $LenovoKnownModel to Lenovo known models" -Severity 1
						}
					}
					global:Write-CMLogEntry -Value "Info: Found: $(($LenovoKnownProducts).count) known Lenovo models" -Severity 1
				}
			}
			if (($LenovoModels).Count -gt "0") {
				global:Write-CMLogEntry -Value "Info: Found $(($LenovoModels).count) Lenovo model driver packs for $($OSComboBox.text) $($ArchitectureComboxBox.text)" -Severity 1
			}
			else {
				global:Write-CMLogEntry -Value "Warning: No Lenovo models found. If you are using a proxy server please specify the proxy in the Proxy Server Settings tab." -Severity 2
			}
		}
		if ($MicrosoftCheckBox.Checked -eq $true) {
			...
			# Find Models Contained Within Downloaded XML		
			if ($OSComboBox.SelectedItem -match "Windows 10") {
				$OSSelected = "Win10"
				
				# Microsoft Windows 10 codename matching
				
				switch -wildcard ($($OSComboBox.SelectedItem)) {
					"*1809" {
						$global:Win10Codename = "RS5"
					}
					"*1803" {
						$global:Win10Codename = "RS4"
					}
					"*1709" {
						$global:Win10Codename = "RS3"
					}
					"*1703" {
						$global:Win10Codename = "RS2"
					}
				}
				
				Write-Verbose "Models = $MicrosoftModelList"
				$MicrosoftModels = $MicrosoftModelList.Drivers.Model | Where-Object {
					($_.OSSupport.Name -match $OSSelected) -and ($_.OSSupport.Win10Build.Name -match $global:Win10Codename)
				} | Sort-Object
			...
			if ($OSComboBox.SelectedItem -eq "Windows 8.1") {
				$OSSelected = "Win81"
				$MicrosoftModels = $MicrosoftModelList.Drivers.Model | Where-Object {
					$_.OSSupport.Name -match $OSSelected
				} | Sort-Object
			}
			...
			if ($MicrosoftModels -ne $null) {
				...
				foreach ($Model in $MicrosoftModels.name) {
					if ($MicrosoftKnownProducts -notcontains $Model) {
						$MicrosoftKnownProducts.Add($Model) | Out-Null
						global:Write-CMLogEntry -Value "Info: Adding $Model to Microsoft known models" -Severity 1
					}
				}
				global:Write-CMLogEntry -Value "Info: Found: $(($MicrosoftKnownProducts).count) known Microsoft models" -Severity 1
				$StartDownloadButton.Enabled = $true
			}
			$MicrosoftKnownProducts = $MicrosoftKnownProducts | Sort-Object
			if ($MicrosoftKnownProducts -ne $null) {
				foreach ($MicrosoftModel in $MicrosoftKnownProducts) {
					$MakeModelDataGrid.Rows.Add($false, "Microsoft", $MicrosoftModel, $OSComboBox.Text, $ArchitectureComboxBox.Text)
				}
			}
			if (($MicrosoftModels).Count -gt "0") {
				global:Write-CMLogEntry -Value "Info: Found $(($MicrosoftModels).count) Microsoft model driver packs for $($OSComboBox.text) $($ArchitectureComboxBox.text)" -Severity 1
			}
			else {
				global:Write-CMLogEntry -Value "Info: No Microsoft models Found. If you are using a proxy server please specify the proxy in the Proxy Server Settings tab" -Severity 2
			}
		}
		$XMLLoading.Visible = $false
		$XMLLoadingLabel.Visible = $false
		if ($ConfigMgrImport.text -eq "yes") {
			global:Write-CMLogEntry -Value "======== Adding ConfigMgr Known Models ========" -Severity 1
			if ($DellKnownModels -ne $null) {
				SelectKnownModels -SearchMake "Dell"
			}
			if ($HPKnownModels -ne $null) {
				SelectKnownModels -SearchMake "Hewlett-Packard"
			}
			if ($LenovoKnownModels -ne $null) {
				SelectKnownModels -SearchMake "Lenovo"
			}
			if ($MicrosoftKnownModels -ne $null) {
				SelectKnownModels -SearchMake "Microsoft"
			}
		}
		
		# Loop for each seleted model
		if ($XMLSelectedModels -ne $null) {
			global:Write-CMLogEntry -Value "======== Selecting Previously Selected Models ========" -Severity 1
			for ($Row = 0; $Row -lt $MakeModelDataGrid.RowCount; $Row++) {
				foreach ($XMLSelectedModel in $XMLSelectedModels) {
					if ($MakeModelDataGrid.Rows[$Row].Cells[2].Value -eq $XMLSelectedModel) {
						$MakeModelDataGrid.Rows[$Row].Cells[0].Value = $true
						$MakeModelDataGrid.Rows[$Row].Selected = $true
						global:Write-CMLogEntry -Value "Info: Selecting model $XMLSelectedModel" -Severity 1
					}
					else {
			...
			if ($OS -match "10") {
				if (-not ([string]::IsNullOrEmpty($OSVersion))) {
					...
					if (-not ([string]::IsNullOrEmpty($Win10VersionLink))) {
						...
						return ($MatchingLink | Select-Object -First 1).Trim()
					}
				}
				else {
					return ($MatchingLink | Select-Object -First 1).Trim()
	...
	function DiscoverDPOptions {
		global:Write-CMLogEntry -Value "======== Querying ConfigMgr Distribution Options ========" -Severity 1
		Set-Location -Path ($SiteCode + ":")
		...
		if ($DistributionPoints -ne $null) {
			foreach ($DP in $DistributionPoints) {
				$DP = ($DP).TrimStart("\\")
				global:Write-CMLogEntry -Value "Info: Adding Distribution Point - $DP" -Severity 1
			...
			global:Write-CMLogEntry -Value "Info: Found $($DistributionPoints.Count) Distribution Points" -Severity 1
		}
		...
		if ($DistributionPointGroups -ne $null) {
			foreach ($DPG in $DistributionPointGroups) {
				global:Write-CMLogEntry -Value "Info: Adding Distribution Point Group - $DPG" -Severity 1
			...
			global:Write-CMLogEntry -Value "Info: Found $($DistributionPointGroups.Count) Distribution Point Groups" -Severity 1
		}
		Set-Location -Path $global:TempDirectory
	}
	
	function ConfigMgrOptions {
		param
		(
			[parameter(Mandatory = $true)]
	...
	function MDTOptions {
		...
		if ($OptionsEnabled -eq $true) {
			global:Write-CMLogEntry -Value "Info: Enabling MDT Options" -Severity 1
		}
		else {
			global:Write-CMLogEntry -Value "Info: Disabling MDT Options" -Severity 1
		}
	...
	function DistributeContent {
		...
		for ($Row = 0; $Row -lt $DPGridView.RowCount; $Row++) {
			if ($DPGridView.Rows[$Row].Cells[0].Value -eq $true) {
				...
				global:Write-CMLogEntry -Value "$($Product): Distributing Package $PackageID to Distribution Point - $($DPGridView.Rows[$Row].Cells[1].Value) " -Severity 1
			}
		}
		...
		for ($Row = 0; $Row -lt $DPGGridView.RowCount; $Row++) {
			if ($DPGGridView.Rows[$Row].Cells[0].Value -eq $true) {
				...
				global:Write-CMLogEntry -Value "$($Product): Distributing Package $PackageID to Distribution Point Group - $($DPGGridView.Rows[$Row].Cells[1].Value) " -Severity 1
			}
		}
	}
	
	function ConnectConfigMgr {
		...
		if (-not ([string]::IsNullOrEmpty($SiteServer))) {
			...
			if ((Test-WSMan -ComputerName $SiteServer).wsmid -ne $null) {
				...
				$ProgressListBox.ForeColor = "Black"
				try {
					global:Write-CMLogEntry -Value "======== Connecting to ConfigMgr Server ========" -Severity 1
					global:Write-CMLogEntry -Value "Info: Querying site code From $SiteServer" -Severity 1
					...
					if ($env:SMS_ADMIN_UI_PATH -ne $null) {
						...
						global:Write-CMLogEntry -Value "Info: Loading ConfigMgr PowerShell module" -Severity 1
				...
				ErrorOutput -Message "Error: ConfigMgr server specified not found - $($SiteServerInput.Text)" -Severity 3
			}
		}
		else {
			ErrorOutput -Message "Error: ConfigMgr site server not specified. Please review in the common settings tab." -Severity 3
		}
	}
	...
	function DellBiosFinder {
		...
		if ((Test-Path -Path $global:TempDirectory\$DellCatalogXMLFile) -eq $false) {
			global:Write-CMLogEntry -Value "======== Downloading Dell XML Catalog  ========" -Severity 1
			global:Write-CMLogEntry -Value "Info: Downloading Dell XML catalog cabinet file from $DellCatalogSource" -Severity 1
			# Download Dell Model Cabinet File
			try {
				if ($global:ProxySettingsSet -eq $true) {
				...
				# Expand Cabinet File
				global:Write-CMLogEntry -Value "Info: Expanding Dell XML catalog cabinet file: $DellCatalogFile" -Severity 1
		...
		if ((Test-Path -Path $global:TempDirectory\$DellCatalogXMLFile) -eq $true) {
			if ($global:DellCatalogXML -eq $null) {
				# Read XML File
				global:Write-CMLogEntry -Value "Info: Reading Dell product XML file - $global:TempDirectory\$DellCatalogXMLFile" -Severity 1
			...
			if ($SKU -notmatch ";") {
				...
				global:Write-CMLogEntry -Value "Info: Attempting to match based on multiple model package" -Severity 1
				$DellBIOSFile = $global:DellCatalogXML.Manifest.SoftwareComponent | Where-Object {
			...
			if (($DellBIOSFile -eq $null) -or (($DellBIOSFile).Count -gt 1)) {
				global:Write-CMLogEntry -Value "Info: Attempting to find BIOS link" -Severity 1
			...
			ErrorOutput -Message "Error: Issues occured while extracting XML file" -Severity 1
			Return "Badlink"
		}
	}
	
	function HPBiosFinder {
		param (
			[string]$Model,
			[string]$OS,
			[string]$Architecture,
		...
		global:Write-CMLogEntry -Value "Info: Checking for existing HP cabinet file $HPPlatformCabFile" -Severity 1
		if ((Test-Path -Path $(Join-Path -Path $global:TempDirectory -ChildPath $HPPlatformCabFile)) -eq $false) {
			try {
				Set-Location -Path $global:TempDirectory
				# Download HP Model Details XML
				
				global:Write-CMLogEntry -Value "Info: Downloading HP XML from $HPPlatFormList" -Severity 1
				if ($global:ProxySettingsSet -eq $true) {
				...
				if ((Test-Path -Path $(Join-Path $global:TempDirectory -ChildPath $HPPlatformXMLFile)) -eq $false) {
					# Expand Cabinet File
					global:Write-CMLogEntry -Value "Info: Expanding HP cabinet file: $HPXMLFile" -Severity 1
					Expand "$global:TempDirectory\$HPPlatformCabFile" -F:* "$global:TempDirectory\$HPPlatformXMLFile" | Out-Null
					}
			}
			catch {
				ErrorOutput -Message "Error: $($_.Exception.Message)" -Severity 3
		...
		global:Write-CMLogEntry -Value "Info: Reading HP XML from $(Join-Path -Path $global:TempDirectory -ChildPath ($HPPlatformXMLFile | Split-Path -Leaf))" -Severity 1
		...
		if ($global:HPPlatformXML -ne $null) {
			global:Write-CMLogEntry -Value "Info: OS pre build strip is $OS" -Severity 1
			global:Write-CMLogEntry -Value "Info: Model is $Model" -Severity 1
			if ($HPSystemSKU -ne $null) {
				...
				global:Write-CMLogEntry -Value "Info: SystemID is $HPSystemSKU" -Severity 1
				global:Write-CMLogEntry -Value "Info: OS is $OS" -Severity 1
				global:Write-CMLogEntry -Value "Info: Architecture is $Architecture" -Severity 1
				$HPXMLCabinetSource = "http://ftp.hp.com/pub/caps-softpaq/cmit/imagepal/ref/" + $($($HPSystemSKU.Split(",") | select -First 1) + "/" + $($HPSystemSKU.Split(",") | select -First 1) + "_" + $($Architecture.TrimStart("x")) + "_" + $OS + ".cab")
				global:Write-CMLogEntry -Value "Info: URL is $HPXMLCabinetSource" -Severity 1
				...
				try {
					...
					ErrorOutput -Message "Error: An error occurred while attempting contact $HPXMLCabinetSource - $($_.Exception.Message)" -Severity 2
				}
				if ($HPDownloadError -ne $true) {
					...
					if ((Test-Path -Path $(Join-Path -Path $global:TempDirectory -ChildPath $HPCabFile)) -eq $true) {
						# Expand Cabinet File
						global:Write-CMLogEntry -Value "Info: Expanding HP SoftPaq cabinet file: $HPCabFile" -Severity 1
						Expand "$global:TempDirectory\$HPCabFile" -F:* "$global:TempDirectory\$HPXMLFile" | Out-Null
						
						# Read HP Model XML
						global:Write-CMLogEntry -Value "Info: Reading model XML $HPXMLFile" -Severity 1
						...
						global:Write-CMLogEntry -Value "Info: BIOS download URL is $($HPBIOSDetails.URL)" -Severity 1
						Return $HPBIOSDetails
					}
					else {
						ErrorOutput -Message "Error: Failed to download $HPXMLCabinetSource" -Severity 1
					}
				}
				else {
					global:Write-CMLogEntry -Value "Info: Could not download HP $Model BIOS update" -Severity 1
				}
			}
		}
	}
	...
	function LenovoBiosFinder {
		...
		try {
			...
			global:Write-CMLogEntry -Value "Lenovo Base - $LenovoBiosBase, Lenovo Model Type $LenovoModelType" -Severity 1
			global:Write-CMLogEntry -Value "Info: Quering XML $($LenovoBiosBase + $LenovoModelType + "_Win$OS.xml") for BIOS download links " -Severity 1
	...
	function BitsJobMonitor {
		...
		global:Write-CMLogEntry -Value "BitsTransfer: Checking BITS background job" -Severity 1 -SkipGuiLog $true
		
		$BitsJob = Get-BitsTransfer | Where-Object {
			$_.DisplayName -match "$BitsJobName"
		}
		while (($BitsJob).JobState -eq "Connecting") {
			global:Write-CMLogEntry -Value "BitsTransfer: Establishing connection to $DownloadSource" -Severity 1
			Start-Sleep -seconds 5
		}
		...
		while (($BitsJob).JobState -eq "Transferring") {
			if ($BitsJob.BytesTotal -ne $null) {
				...
				global:Write-CMLogEntry -Value "BitsTransfer: Downloaded $([int]((($BitsJob).BytesTransferred)/ 1MB)) MB of $([int]((($BitsJob).BytesTotal)/ 1MB)) MB ($PercentComplete%). Next update in 30 seconds." -Severity 1
				Start-Sleep -seconds 30
			}
			else {
				global:Write-CMLogEntry -Value "BitsTransfer: Download issues detected. Cancelling download process" -Severity 2
	...
	function Read-XMLSettings {
		global:Write-CMLogEntry -Value "======== Reading Settings File ========" -Severity 1
		...
		try {
			...
			global:Write-CMLogEntry -Value "Setting ConfigMgr Site Settings" -Severity 1
			...
			global:Write-CMLogEntry -Value "Setting OS & Download Selections" -Severity 1
			...
			if ($global:DATSettingsXML.Settings.PackageSettings.CustomEnabled -eq $true) {
				global:Write-CMLogEntry -Value "Setting Custom Package Location" -Severity 1
				$SpecifyCustomPath.Enabled = $true
			...
			# // Storage Locations
			global:Write-CMLogEntry -Value "Setting Storage Locations" -Severity 1
			...
			global:Write-CMLogEntry -Value "Setting Manufacturer Selections" -Severity 1
			...
			if (($global:DATSettingsXML.Settings.Manufacturer.Lenovo -eq "True") -and ($global:LenovoDisable -eq $false)) {
				$LenovoCheckBox.Checked = $true
			}
			...
			global:Write-CMLogEntry -Value "Import ConfigMgr Models Setting" -Severity 1
			...
			# // Clean Up Options	
			global:Write-CMLogEntry -Value "Setting Clean Up Settings" -Severity 1
			if ($global:DATSettingsXML.Settings.CleanUpOptions.CleanUnused -eq "True") {
			...
			if ($global:DATSettingsXML.Settings.CleanUpOptions.RemoveLegacyBIOS -eq "True") {
				$RemoveLegacyBIOSCheckbox.Enabled = $true
			...
			if ($global:DATSettingsXML.Settings.ProxySettings.UseProxy -eq "True") {
				global:Write-CMLogEntry -Value "Enabling proxy server options" -Severity 1
				$UseProxyServerCheckbox.Checked = $true
				global:Write-CMLogEntry -Value "Setting proxy server address to $($global:DATSettingsXML.Settings.ProxySetting.Proxy)" -Severity 1
			...
			global:Write-CMLogEntry -Value "Setting Previously Selected Distribution Points / Distribution Point Groups" -Severity 1
			...
			# Connect to SCCM Site if selected platform is not MDT
			...
			# Model Selections
			global:Write-CMLogEntry -Value "Setting Previously Selected Model(s)" -Severity 1
			foreach ($Model in $global:DATSettingsXML.Settings.Models.ModelSelected) {
				$XMLSelectedModels.Add($Model)
			}
		}
		catch {
			global:Write-CMLogEntry -Value "An error occurred while attempting to apply settings from DATSettings XML: $($_.Exception.Message)" -Severity 2
		}
	...
	function Write-XMLModels {
		...
		if ((Test-Path -Path $XMLPath) -eq $false) {
			New-Item -Path $XMLPath -Type Dir -Force
		}
		...
		if ((Test-Path -Path $Path) -eq $false) {
			...
			global:Write-CMLogEntry -Value "XML Model List : Creating XML models file in location - $Path" -Severity 1
			...
			$xmlWriter.WriteEndElement()
			global:Write-CMLogEntry -Value "XML Model List : Adding $Model to XML models file" -Severity 1
			...
			if ($Model -notin $xmlDoc.Details.ModelDetails.Model) {
				...
				# Save XML Document
				$xmlDoc.Save($Path);
				global:Write-CMLogEntry -Value "XML Model List : Appending $Model to XML models file $Path" -Severity 1
			}
			else {
				global:Write-CMLogEntry -Value "XML Model List : $Model already listed in XML models file $Path" -Severity 1
			}
		}
	}
	...
	function InitiateDownloads {
		...
		global:Write-CMLogEntry -Value "Info: Importing Into Products: $ImportInto" -Severity 1
		$DownloadType = [string]$DownloadComboBox.SelectedItem
		global:Write-CMLogEntry -Value "Info: Download Type: $DownloadType" -Severity 1
		...
		if ((($ImportModels.Count) -lt "1") -and (($global:HPSoftPaqDownloads.Count) -lt "1")) {
			ErrorOutput -Message "Error: No models or softpaqs selected" -Severity 3
			$ValidationErrors++
		}
		
		# Validate Download Path
		if ([string]::IsNullOrEmpty($DownloadPathTextBox.Text)) {
			ErrorOutput -Message "Error: Download path not specified on ConfigMgr Settings tab" -Severity 3
			$ValidationErrors++
		}
		...
		if ($DownloadPathTextBox.Text -ne $PackagePathTextBox.Text) {
			...
			if ((Test-Path -Path $DownloadPathTextBox.Text) -eq $true) {
				...
				global:Write-CMLogEntry -Value "Pre-Check: Download path set To $DownloadPath" -Severity 1
			}
			else {
				ErrorOutput -Message "Error: UNC download path specified could not be found $($DownloadPathTextBox.Text)" -Severity 3
				$ValidationErrors++
			}
			...
			if (($ImportInto -like "ConfigMgr*") -or ($ImportInto -like "Both*")) {
				if (![string]::IsNullOrEmpty($PackagePathTextBox.Text)) {
					if ((Test-Path -path $PackagePathTextBox.Text) -eq $true) {
						...
						ErrorOutput -Message "Error: UNC package path specified could not be found $($PackagePathTextBox.Text)" -Severity 3
						$ValidationErrors++
					}
				}
				else {
					ErrorOutput -Message "Error: Package path is empty" -Severity 3
					$ValidationErrors++
				}
			}
		}
		else {
			ErrorOutput -Message "Error: Download and package paths must be different." -Severity 3
			$ValidationErrors++
		}
		...
		if (($OSComboBox).Text -ne $null) {
			...
			ErrorOutput -Message "Error: Operating System not specified" -Severity 3
			$ValidationErrors++
		}
		...
		if (($ArchitectureComboxBox).Text -ne $null) {
			...
			ErrorOutput -Message "Error: Operating System architecture not specified" -Severity 3
			$ValidationErrors++
		}
		...
		if ($ImportInto -match "MDT") {
			...
			if ($DeploymentShareCount -eq 0) {
				ErrorOutput -Message "Error: No MDT deployment shares selected. Please select at least one deployment share." -Severity 3
				$ValidationErrors++
			}
		}
		...
		$DriverExtractJob = {
			...
			try {
				Expand $DriverSourceCab -F:* $DriverExtractDest | Out-Null
			}
			catch [System.Exception] {
				ErrorOutput -Message "Error: $($_.Exception.Message)" -Severity 3
			}
		...
		$PackageDrivers = {
			...
			try {
				if ($Make -eq "Dell") {
					...
					global:Write-CMLogEntry -Value "$($Product): Copying drivers from $DriverExtractDest to $DriverPackageDest" -Severity 1
				}
				else {
				...
				ErrorOutput -Message "Error: $($_.Exception.Message)" -Severity 3
				ErrorOutput -Message "Error: Check security permissions and specified path" -Severity 3
			}
		}
		...
		if ((($ImportInto) -like ("MDT" -or "Both*")) -and ($ValidationErrors -eq 0)) {
			...
			if ((Test-Path -Path $MDTPSCommandlets) -eq $true) {
				# Import MDT Module
				global:Write-CMLogEntry -Value "$($Product): Importing: MDT PowerShell Commandlets" -Severity 1
				Import-Module $MDTPSCommandlets
			}
			else {
				ErrorOutput -Message "Error: MDT PowerShell Commandlets file not found at $MDTPSCommandlets" -Severity 1
				$ValidationErrors++
			}
		}
		...
		if ($ValidationErrors -eq 0) {
			...
			if ($ProductListBox.SelectedItems -ge 1) {
				#global:Write-CMLogEntry -Value "Info: Models selected: $($ProductListBox.SelectedItems)" -Severity 1
				global:Write-CMLogEntry -Value "Info: Models selected: $($ImportModels)" -Severity 1
			}
			else {
				#global:Write-CMLogEntry -Value "Info: Models selected: $($ProductListBox.Items)" -Severity 1
				global:Write-CMLogEntry -Value "Info: Models selected: $($ImportModels)" -Severity 1
			}
			...
			global:Write-CMLogEntry -Value "Info: Site Code specified: $($SiteCode)" -Severity 1
			global:Write-CMLogEntry -Value "Info: Download Path specified: $($DownloadPath)" -Severity 1
			global:Write-CMLogEntry -Value "Info: Package Path specified: $($PackagePath)" -Severity 1
			...
			foreach ($Model in $ImportModels) {
				global:Write-CMLogEntry -Value "======== Processing $Model Downloads ========" -Severity 1
				# Vendor Make
				$Make = $($Model).split(" ")[0]
				...
				if ($OSComboBox.Text -like "Windows 10 1*") {
					...
					global:Write-CMLogEntry -Value "Info: Windows 10 build $OSBuild and version $OSVersion identified for driver match" -Severity 1
				}
				else {
					$OSVersion = ([string]($OSComboBox).Text).Split(' ')[1]
				}
				
				global:Write-CMLogEntry -Value "Info: Starting Download, extract and import processes for $Make model: $($Model)" -Severity 1
				...
				if ($Make -eq "Dell") {
					...
					$DellSystemSKU = (($global:DellModelCabFiles.supportedsystems.brand.model | Where-Object {
								$_.Name -eq $Model
							}).systemID) | Select-Object -Unique
					$ModelURL = $DellDownloadBase + "/" + ($global:DellModelCabFiles | Where-Object {
							((($_.SupportedOperatingSystems).OperatingSystem).osCode -match $WindowsVersion) -and ($_.SupportedSystems.Brand.Model.SystemID -match $DellSystemSKU)
						}).delta
					if ($DellSystemSKU.Count -gt 1) {
						$DellSingleSKU = $DellSystemSKU | select -First 1
						$DellSystemSKU = [string]($DellSystemSKU -join ";")
						global:Write-CMLogEntry -Value "Info: Using SKU : $DellSingleSKU" -Severity 1
						$ModelURL = $DellDownloadBase + "/" + ($global:DellModelCabFiles | Where-Object {
				...
				if ($Make -eq "Hewlett-Packard") {
					global:Write-CMLogEntry -Value "Info: Setting Hewlett-Packard variables" -Severity 1
					if ($global:HPModelSoftPaqs -eq $null) {
						[xml]$global:HPModelXML = Get-Content -Path $global:TempDirectory\$HPXMLFile
						# Set XML Object
						$global:HPModelXML.GetType().FullName
				...
				if ($Make -eq "Lenovo") {
					...
					global:Write-CMLogEntry -Value "Info: $Make $Model matching model type: $global:LenovoModelType" -Severity 1 -SkipGuiLog $false
					...
					if ($WindowsVersion -ne "7") {
						global:Write-CMLogEntry -Value "Info: Looking Up Lenovo $Model URL For Windows version win$(($WindowsVersion).Trim('.'))" -Severity 1
					...
					if ($DownloadType -ne "BIOS") {
						global:Write-CMLogEntry -Value "Info: Searching for Lenovo $Model exe file on $ModelURL" -Severity 1
						...
						[string]$DriverDownload = FindLenovoDriver -URI $ModelURL -os $($WindowsVersion -replace '[.]', '') -Architecture $Architecture -OSVersion $OSVersion
						If (($DriverDownload -ne $null) -and ($DriverDownload -notmatch "badlink")) {
							global:Write-CMLogEntry -Value "Info: Driver package URL - $DriverDownload" -Severity 1
							$DriverCab = $DriverDownload | Split-Path -Leaf
							$DriverRevision = ($DriverCab.Split("_") | Select-Object -Last 1).Trim(".exe")
						}
						else {
							ErrorOutput -Message "Error: Unable to find driver for $Make $Model" -Severity 3
						}
					}
				}
				if ($Make -eq "Microsoft") {
					...
					$ModelURL = ((($MicrosoftModelDrivers.Model | Where-Object {
									($_.name -eq "$Model")
					...
					$ModelURL = ($DriverDownload = (((($MicrosoftModelDrivers.Model | Where-Object {
											$_.Name -eq $Model
										}).OSSupport) | Where-Object {
									$_.Name -match "$OS"
								}).Win10Build | Where-Object {
								$_.Name -match "$Win10Codename"
							} | Select-Object -ExpandProperty DownloadURL))
					
					$DriverCab = $DriverDownload | Split-Path -Leaf
					$DriverRevision = ($DriverCab.Split("_") | Select-Object -Last 2).Trim(".msi")[0]
				}
				
				# Generalise SKU / BaseBoard Matching Value
				if ($Make -match "Dell") {
					$SkuValue = $DellSystemSKU
				}
				elseif ($Make -match "Hewlett") {
					$SkuValue = $HPSystemSKU
				}
				elseif ($Make -match "Microsoft") {
					$SkuValue = $MSSystemSKU
				}
				elseif ($Make -match "Lenovo") {
					$SkuValue = $global:LenovoSystemSKU
				}
				...
				if ($ImportInto -ne "MDT") {
					...
					if ($DownloadType -ne "Drivers") {
						global:Write-CMLogEntry -Value "======== $MODEL BIOS PROCESSING STARTED ========" -Severity 1
						...
						if ($Make -eq "Dell") {
							...
							$DellBIOSDownload = DellBiosFinder -SKU $DellSystemSKU
							if ($DellBIOSDownload -notcontains "BadLink") {
								...
								global:Write-CMLogEntry -Value "Info: Latest available BIOS version is $BIOSVer" -Severity 1
								$BIOSFile = $DellBIOSDownload.Path | Split-Path -Leaf
								$BIOSVerDir = $BIOSVer -replace '\.', '-'
								if ($ImportInto -match "Download") {
								...
								if ($Product -match "Download") {
									global:Write-CMLogEntry -Value "Info: Checking for existing BIOS release - $BIOSVer" -Severity 1
								...
								elseif ($Product -eq "ConfigMgr") {
									Set-Location -Path ($SiteCode + ":")
									global:Write-CMLogEntry -Value "Info: Checking ConfigMgr for existing BIOS release - $BIOSVer" -Severity 1
									$CurrentBIOSPackage = Get-CMPackage -Name $BIOSUpdatePackage | Select-Object Name, PackageID, SourceDate, Version | Sort-Object SourceDate -Descending | Select-Object -First 1
									if (![string]::IsNullOrEmpty($CurrentBIOSPackage.Version)) {
										global:Write-CMLogEntry -Value "Info: Comparing BIOS versions" -Severity 1
										if ($BIOSVer -ne $CurrentBIOSPackage.Version) {
											$NewBIOSAvailable = $true
											global:Write-CMLogEntry -Value "Info: New BIOS download available" -Severity 1
										}
										else {
											$NewBIOSAvailable = $false
											global:Write-CMLogEntry -Value "Info: BIOS package already exists for $Make $Model (Version $BIOSVer)." -Severity 2
										}
									}
									else {
										$NewBIOSAvailable = $true
								...
								If ($NewBIOSAvailable -eq $true) {
									if (($BIOSDownload -like "*.exe") -and ($Make -eq "Dell")) {
										global:Write-CMLogEntry -Value "Info: BIOS Download URL Found: $BIOSDownload" -Severity 1
										...
										if ((Test-Path -Path "$($DownloadRoot + $Model + '\BIOS\' + $BIOSVerDir + '\' + $BIOSFile)") -eq $false) {
											If ((Test-Path -Path $BIOSUpdateRoot) -eq $false) {
												global:Write-CMLogEntry -Value "Info: Creating $BIOSUpdateRoot folder" -Severity 1
												New-Item -Path $BIOSUpdateRoot -ItemType Directory
											}
											global:Write-CMLogEntry -Value "Info: Downloading $($BIOSFile) BIOS update file" -Severity 1
											try {
												if ($global:ProxySettingsSet -eq $true) {
											...
											global:Write-CMLogEntry -Value "Info: Skipping $BIOSFile... File already downloaded." -Severity 2
											$DownloadSuccess = $true
										}
										if ($DownloadSuccess -eq $true) {
											...
											if ((Test-Path -Path $Flash64BitZip) -eq $false) {
												if ((Test-Path -Path $FlashUtilDir) -eq $false) {
													global:Write-CMLogEntry -Value "Info: Creating Directory - $FlashUtilDir" -Severity 1
													New-Item -ItemType Directory -Path $FlashUtilDir | Out-Null
												}
												global:Write-CMLogEntry -Value "Info: Downloading $Make flash update utility" -Severity 1
												try {
													if ($global:ProxySettingsSet -eq $true) {
													...
													ErrorOutput -Message "Error: $($_.Exception.Message)" -Severity 3
													ErrorOutput -Message "Error: BIOS flash utility failed to download. Check log for more details" -Severity 3
												}
											}
											if ((Test-Path -Path $Flash64BitZip) -eq $true) {
												$Flash64BitExe = ($Flash64BitZip -replace ".zip", ".exe")
												if ((Test-Path -Path $Flash64BitExe) -eq $false) {
													# Unzip Flash64 Exe
													global:Write-CMLogEntry -Value "Info: Unzipping Dell Flash64 EXE" -Severity 1
													Add-Type -AssemblyName "system.io.compression.filesystem"
													[io.compression.zipfile]::ExtractToDirectory("$($Flash64BitZip)", "$($FlashUtilDir)")
												}
												global:Write-CMLogEntry -Value "Info: Copying Dell Flash64Bit EXE To $BIOSUpdateRoot" -Severity 1
												Get-Item -Path $Flash64BitExe | Copy-Item -Destination "$($BIOSUpdateRoot)"
												if (($Product -ne "Download Only") -and ((Test-Path -Path "$($BIOSUpdateRoot + $BIOSFile)")) -eq $true) {
													# ================= Create BIOS Update Package ==================			
													Set-Location -Path ($SiteCode + ":")
													$BIOSModelPackage = Get-CMPackage -Name $BIOSUpdatePackage | Select-Object SourceDate, Version | Sort-Object SourceDate -Descending | Select-Object -First 1
													if (($BIOSModelPackage.Version -ne $BIOSVer) -or ($BIOSModelPackage -eq $null)) {
														global:Write-CMLogEntry -Value "$($Product): Creating BIOS Package" -Severity 1
														New-CMPackage -Name "$BIOSUpdatePackage" -Path "$BIOSUpdateRoot" -Description "$Make $Model BIOS Update (Models included:$DellSystemSKU)" -Manufacturer "$Make" -Language English -version $BIOSVer
														if ($EnableBinaryDifCheckBox.Checked -eq $true) {
															global:Write-CMLogEntry -Value "$($Product): Enabling Binary Delta Replication" -Severity 1
															Set-CMPackage -Name "$BIOSUpdatePackage" -EnableBinaryDeltaReplication $true -Priority "$($DistributionPriorityCombo.Text)"
														}
														$SCCMPackage = Get-CMPackage -Name $BIOSUpdatePackage | Select-Object PackageID, Version, Name | Where-Object {
														...
														# =============== Distrubute Content =================
														Set-Location -Path ($SiteCode + ":")
														$SCCMPackage = Get-CMPackage -Name $BIOSUpdatePackage | Select-Object PackageID, Version | Where-Object {
															$_.Version -eq $BIOSVer
														}
														Move-CMObject -FolderPath $global:VendorBIOSFolder -ObjectID $SCCMPackage.PackageID
														DistributeContent -Product $Product -PackageID $SCCMPackage.PackageID -ImportInto $ImportInto
														Set-Location -Path $global:TempDirectory
													}
												}
											}
											else {
												ErrorOutput -Message "Error: BIOS flash upgrade utility failed to download. Check log for more details" -Severity 3
											}
										}
										else {
											ErrorOutput -Message "Error: BIOS failed to download. Check log for more details" -Severity 3
										}
									}
									else {
										global:Write-CMLogEntry -Value "Info: Unable to retrieve BIOS download URL For $Make Client Model: $($Model)" -Severity 2
									}
								}
								else {
									global:Write-CMLogEntry -Value "Info: Current BIOS package already exists - $($CurrentBIOSPackage.Name) - $($CurrentBIOSPackage.Version) ($($CurrentBIOSPackage.PackageID))" -Severity 1
								}
							}
						}
						if ($Make -eq "Lenovo") {
							# ================= Lenovo BIOS Upgrade Download ==================
							global:Write-CMLogEntry -Value "Info: Retrieving BIOS download URL for $Make Client Model: $($Model)" -Severity 1
							Set-Location -Path $global:TempDirectory
							global:Write-CMLogEntry -Value "Info: Attempting to find download URL using LenovoBiosFinder function" -Severity 1
							$BIOSDownload = LenovoBiosFinder -Model $Model -OS $WindowsVersion
							if ($BIOSDownload.Location -ne $null) {
								global:Write-CMLogEntry -Value "Info: Downloading BIOS update from $($BIOSDownload.Location) " -Severity 1
								...
								if ($LenovoBIOSDetails.Name -ne $null) {
									...
									global:Write-CMLogEntry -Value "Info: Found exe file link: $BIOSFile" -Severity 1
									...
									global:Write-CMLogEntry -Value "Info: BIOS version is $BIOSVer" -Severity 1
									if ($Product -match "Download") {
										if ((Test-Path -Path $BIOSUpdateRoot) -eq $true) {
											if ((Get-ChildItem -Path $BIOSUpdateRoot -File) -contains $BIOSFile) {
												$NewBIOSAvailable = $false
									...
									elseif ($Product -eq "ConfigMgr") {
										Set-Location -Path ($SiteCode + ":")
										global:Write-CMLogEntry -Value "Info: Checking ConfigMgr for existing BIOS release - $BIOSVer" -Severity 1
										$CurrentBIOSPackage = Get-CMPackage -Name $BIOSUpdatePackage | Select-Object PackageID, SourceDate, Version | Sort-Object SourceDate -Descending | Select-Object -First 1
										if (![string]::IsNullOrEmpty($CurrentBIOSPackage.Version)) {
											global:Write-CMLogEntry -Value "Info: Comparing BIOS versions" -Severity 1
											if ($BIOSVer -ne $CurrentBIOSPackage.Version) {
												$NewBIOSAvailable = $true
									...
									if ($NewBIOSAvailable -eq $true) {
										global:Write-CMLogEntry -Value "Info: BIOS update directory set to $BIOSUpdateRoot" -Severity 1
										# Check for destination directory, create if required and download the BIOS upgrade file
										if ((Test-Path -Path "$($BIOSUpdateRoot)") -eq $false) {
											New-Item -Path "$BIOSUpdateRoot" -ItemType Directory
										}
										...
										global:Write-CMLogEntry -Value "Info: Downloading BIOS update file from $BIOSFileDownload" -Severity 1
										try {
											if ($global:ProxySettingsSet -eq $true) {
										...
										if ($DownloadSuccess -eq $true) {
											...
											global:Write-CMLogEntry -Value "Info: Unlocking BIOS file located at $($BIOSUpdateRoot + $BIOSFile)" -Severity 1
											global:Write-CMLogEntry -Value "Info: Starting BIOS file extract process" -Severity 1
											global:Write-CMLogEntry -Value "Info: BIOS extract switches used = $BIOSExtractSwitches" -Severity 1
											...
											While ((Get-Process).name -contains $BIOSProcess) {
												global:Write-CMLogEntry -Value "Info: Waiting for extract process (Process: $BIOSProcess) to complete..  Next check in 10 seconds" -Severity 1
												Start-Sleep -Seconds 10
											}
											global:Write-CMLogEntry -Value "Info: Extract process complete" -Severity 1
											global:Write-CMLogEntry -Value "Info: Copying extracted files to $BIOSUpdateRoot" -Severity 1
											Get-ChildItem -Path ($global:TempDirectory + "\$($Model.Replace(' ', ''))\BIOS\$BIOSVer") -Recurse | Move-Item -Destination "$BIOSUpdateRoot" -Force
											global:Write-CMLogEntry -Value "Info: Removing source BIOS exe file" -Severity 1
											...
											If ((Get-ChildItem -Path $BIOSUpdateRoot -File).Count -gt 0) {
												If ($ImportInto -notmatch "Download") {
													...
													Set-Location -Path ($SiteCode + ":")
													global:Write-CMLogEntry -Value "$($Product): Creating BIOS package" -Severity 1
													New-CMPackage -Name "$BIOSUpdatePackage" -Path "$BIOSUpdateRoot" -Description "$Make $Model BIOS Update (Models included:$global:LenovoSystemSKU) (Release Date:$BIOSReleaseDate)" -Manufacturer "$Make" -Language English -version $LenovoBIOSDetails.Version
													if ($EnableBinaryDifCheckBox.Checked -eq $true) {
														global:Write-CMLogEntry -Value "$($Product): Enabling Binary Delta Replication" -Severity 1
														Set-CMPackage -Name "$BIOSUpdatePackage" -EnableBinaryDeltaReplication $true -Priority "$($DistributionPriorityCombo.Text)"
													}
													...
													Move-CMObject -FolderPath $global:VendorBIOSFolder -ObjectID $SCCMPackage.PackageID
													DistributeContent -Product $Product -PackageID $SCCMPackage.PackageID -ImportInto "Standard"
													global:Write-CMLogEntry -Value "$($Product): BIOS update package $($SCCMPackage.PackageID) created & distributing" -Severity 1
												}
											}
											else {
												ErrorOutput -Message "Error: Extract BIOS folder is empty. Issues occured during extraction." -Severity 3
											}
											Set-Location -Path $global:TempDirectory
										}
										else {
										...
										global:Write-CMLogEntry -Value "Info: Current BIOS package already exists - $($CurrentBIOSPackage.PackageID)" -Severity 1
									}
								}
								else {
									ErrorOutput -Message "Error: Unable to find BIOS download for $Make $Model" -Severity 2
								}
							}
							else {
								ErrorOutput -Message "Error: Unable to find BIOS XML link" -Severity 2
							}
							Set-Location -Path $global:TempDirectory
						}
						if ($Make -eq "Hewlett-Packard") {
							# ================= HP BIOS Upgrade Download ==================
							global:Write-CMLogEntry -Value "Info: Attempting to find HP BIOS download" -Severity 1
							$HPBIOSDownload = HPBiosFinder -Model $Model -OS $OSVersion -Architecture $Architecture -HPSystemSKU $($HPSystemSKU.Split(",") | select -First 1)
							if ($HPBIOSDownload.URL -ne $null) {
								...
								elseif ($Product -eq "ConfigMgr") {
									Set-Location -Path ($SiteCode + ":")
									global:Write-CMLogEntry -Value "Info: Checking ConfigMgr for existing BIOS release - $BIOSVer" -Severity 1
									$CurrentBIOSPackage = Get-CMPackage -Name "$BIOSUpdatePackage" | Select-Object SourceDate, Version, PackageID | Sort-Object SourceDate -Descending | Select-Object -First 1
									if (![string]::IsNullOrEmpty($CurrentBIOSPackage.Version)) {
										global:Write-CMLogEntry -Value "Info: Comparing BIOS versions" -Severity 1
								...
								if ($NewBIOSAvailable -eq $true) {
									...
									$HPSilentSwitches = "-PDF -F" + "$HPBIOSTemp\Extract\" + " -S -E"
									if (($BIOSDownload -like "*.exe") -and ($Make -eq "Hewlett-Packard")) {
										global:Write-CMLogEntry -Value "Info: BIOS Download URL Found: $BIOSDownload" -Severity 2
										# Check for destination directory, create if required and download the BIOS upgrade file
										if ((Test-Path -Path "$($BIOSUpdateRoot + $BIOSFile)") -eq $false) {
											If ((Test-Path -Path $HPBIOSTemp) -eq $false) {
												global:Write-CMLogEntry -Value "Info: Creating $HPBIOSTemp folder" -Severity 1
											...
											try {
												...
												if ((Test-Path -Path (Join-Path $HPBIOSTemp $BIOSFile)) -and (Test-Path -Path (Join-Path $HPBIOSTemp $BIOSCVA))) {
													global:Write-CMLogEntry -Value "Info: BIOS file(s) downloaded" -Severity 1
									...
									if ($DownloadSuccess -eq $true) {
										...
										global:Write-CMLogEntry -Value "Info: Unlocking BIOS file located at $HPBIOSSource" -Severity 1
										global:Write-CMLogEntry -Value "Info: Extracting $Make BIOS update to $HPBIOSTemp" -Severity 1
										global:Write-CMLogEntry -Value "Info: Using $Make silent switches: $HPSilentSwitches" -Severity 1
										...
										global:Write-CMLogEntry -Value "Info: HP driver source directory set to $HPBIOSExtract" -Severity 1
										if ((Test-Path -Path "$HPBIOSExtract") -eq $true) {
											...
											while ((Get-Job -Name "$Model-BIOS-Move").State -eq "Running") {
												global:Write-CMLogEntry -Value "Info: Moving $Make $Model $OperatingSystem $Architecture BIOS files. Next check in 10 Seconds" -Severity 1
												Start-Sleep -seconds 10
											}
											$HPExtractComplete = $true
										}
										else {
											ErrorOutput -Message "Error: Issues occurred during the $Make $Model extract process" -Severity 3
											$HPExtractComplete = $false
										}
										[int]$HPPkgFileCount = (Get-ChildItem -Path $BIOSUpdateRoot -Recurse).Count
										global:Write-CMLogEntry -Value "Info: Files in HP extract folder: $HPFileCount" -Severity 1 -SkipGuiLog $true
										global:Write-CMLogEntry -Value "Info: Files in HP BIOS folder: $HPPkgFileCount" -Severity 1 -SkipGuiLog $true
										if ($Product -ne "Download Only") {
											if ($HPExtractComplete -eq $true) {
												# ================= Create BIOS Update Package ==================
												Set-Location -Path ($SiteCode + ":")
												$BIOSModelPackage = Get-CMPackage -Name "$BIOSUpdatePackage" | Select-Object SourceDate, Version | Sort-Object SourceDate -Descending | Select-Object -First 1
												if (($BIOSModelPackage.Version -ne $BIOSVer) -or ($BIOSModelPackage -eq $null)) {
													global:Write-CMLogEntry -Value "$($Product): Creating BIOS package" -Severity 1
													New-CMPackage -Name "$BIOSUpdatePackage" -Path "$BIOSUpdateRoot" -Description "$Make $Model BIOS Update (Models included:$HPSystemSKU)" -Manufacturer "Hewlett-Packard" -Language English -version $BIOSVer
													Start-Sleep -Seconds 5
													if ($EnableBinaryDifCheckBox.Checked -eq $true) {
														global:Write-CMLogEntry -Value "$($Product): Enabling Binary Delta Replication" -Severity 1
														Set-CMPackage -Name "$BIOSUpdatePackage" -EnableBinaryDeltaReplication $true -Priority "$($DistributionPriorityCombo.Text)"
													...
													global:Write-CMLogEntry -Value "$($Product): BIOS package already exists" -Severity 1
												}
											}
											else {
												global:Write-CMLogEntry -Value "Warning: BIOS folder does not contain all extracted files. " -Severity 1
											}
										}
									}
									else {
								...
								global:Write-CMLogEntry -Value "Warning: Unable to retrieve BIOS Download URL For $Make Client Model: $($Model)" -Severity 2
							}
						}
						Set-Location -Path $global:TempDirectory
						global:Write-CMLogEntry -Value "======== $Model BIOS PROCESSING FINISHED ========" -Severity 1
					}
					...
					if ((![string]::IsNullOrEmpty($DriverDownload)) -and ($DriverDownload -notmatch "badlink")) {
						...
						if (($DownloadType -ne "BIOS") -and ($ImportInto -ne "MDT")) {
							global:Write-CMLogEntry -Value "======== $PRODUCT $Model DRIVER PROCESSING STARTED ========" -Severity 1
							# =============== Driver Cab Download =================				
							global:Write-CMLogEntry -Value "$($Product): Retrieving driver pack site For $Make $Model" -Severity 1
							global:Write-CMLogEntry -Value "$($Product): URL found: $ModelURL" -Severity 1
							global:Write-CMLogEntry -Value "$($Product): Latest driver revision found - $DriverRevision" -Severity 1
							if ($ImportInto -match "ConfigMgr") {
								...
								if ($ImportInto -like "*Standard*") {
									...
									global:Write-CMLogEntry -Value "$($Product): Checking ConfigMgr for driver packages matching - $CMPackage" -Severity 1
									# Allow for test/pilot driver packages
									if ($ImportInto -match "Pilot") {
										$CMPackage = $CMPackage.Replace("Drivers -", "Drivers Pilot -")
									}
									$ExistingPackageID = (Get-CMPackage -Name $CMPackage.Trim() | Select-Object Name, PackageID, Description, Version, SourceDate | Where-Object {
							...
							if ([string]::IsNullOrEmpty($ExistingPackageID)) {
								global:Write-CMLogEntry -Value "$($Product): New driver package detected - Processing" -Severity 1
								if (($ModelURL -ne $null) -and ($DriverDownload -ne "badLink")) {
									...
									if ((Test-Path -Path $("$DownloadRoot" + "$Model" + "\Driver Cab\" + "$DriverCab")) -eq $false) {
										global:Write-CMLogEntry -Value "$($Product): Creating $Model download folder" -Severity 1
										if ((Test-Path -Path $("$DownloadRoot" + "$Model" + "\Driver Cab")) -eq $false) {
											global:Write-CMLogEntry -Value "$($Product): Creating $("$DownloadRoot" + "$Model" + "\Driver Cab") folder " -Severity 1
										...
										Start-Job -Name "$Model-DriverDownload" -ScriptBlock $DriverDownloadJob -ArgumentList ($DownloadRoot, $Model, $DriverCab, $DriverDownload, $global:BitsProxyOptions, $global:BitsOptions, $global:ProxySettingsSet)
										#Start-Sleep -Seconds 5
										...
										global:Write-CMLogEntry -Value "$($Product): Reported file byte size size: $global:BitsJobByteSize" -Severity 1
										global:Write-CMLogEntry -Value "$($Product): Downloaded file byte size:  $((Get-Item -Path $($DownloadRoot + $Model + '\Driver Cab\' + $DriverCab)).Length)" -Severity 1
									...
									if (((Test-Path -Path "$($DownloadRoot + "$Model" + '\Driver Cab\' + $DriverCab)") -eq $true) -and ($DriverCab -ne $null) -and (($global:BitsJobByteSize -eq $((Get-Item -Path $($DownloadRoot + $Model + '\Driver Cab\' + $DriverCab)).Length)) -or ($PreviousDownload -eq $true))) {
										global:Write-CMLogEntry -Value "$($Product): $DriverCab - File exists processing driver package" -Severity 1
										if (![string]::IsNullOrEmpty($global:BitsJobByteSize)) {
											global:Write-CMLogEntry -Value "$($Product): $DriverCab - File size verified" -Severity 1
										}
										...
										if ((Get-ChildItem -Path "$DriverExtractDest" -Recurse -Filter *.inf -File).Count -eq 0) {
											...
											if ($Make -eq "Dell") {
												global:Write-CMLogEntry -Value "$($Product): Extracting $Make drivers to $DriverExtractDest" -Severity 1
												...
												While ((Get-Job -Name "$Make $Model driver extract").State -eq "Running") {
													global:Write-CMLogEntry -Value "$($Product): Waiting for extract process to complete..  Next check in 30 seconds" -Severity 1
													Start-Sleep -Seconds 30
												}
											}
											if ($Make -eq "Hewlett-Packard") {
												...
												global:Write-CMLogEntry -Value "$($Product): Extracting $Make drivers to $HPTemp" -Severity 1
												Unblock-File -Path $($DownloadRoot + $Model + '\Driver Cab\' + $DriverCab)
												$HPSilentSwitches = "-PDF -F" + "$HPTemp" + " -S -E"
												global:Write-CMLogEntry -Value "$($Product): Using $Make silent switches: $HPSilentSwitches" -Severity 1
												...
												global:Write-CMLogEntry -Value "$($Product): HP driver source directory set to $HPExtract" -Severity 1
												if ((Test-Path -Path "$HPExtract") -eq $true) {
													...
													while ((Get-Job -Name "$Model-Driver-Move").State -eq "Running") {
														global:Write-CMLogEntry -Value "$($Product): Moving $Make $Model $OperatingSystem $Architecture driver.. Next check in 30 seconds" -Severity 1
														Start-Sleep -seconds 30
													}
												}
												else {
											...
											if ($Make -eq "Lenovo") {
												...
												global:Write-CMLogEntry -Value "$($Product): Using $Make silent switches: $LenovoSilentSwitches" -Severity 1
												global:Write-CMLogEntry -Value "$($Product): Extracting $Make drivers to $DriverExtractDest" -Severity 1
												Unblock-File -Path $($DownloadRoot + $Model + '\Driver Cab\' + $DriverCab)
											...
											if ($Make -eq "Microsoft") {
												...
												global:Write-CMLogEntry -Value "$($Product): Extracting $Make drivers to $MicrosoftTemp" -Severity 1
												...
												$MicrosoftExtractDirs = Get-ChildItem -Path $MicrosoftTemp -Directory -Recurse | Where-Object {
													$_.Name -match "Drivers" -or $_.Name -match "Firmware"
												}
												# Set Microsoft extracted folder											
												$MicrosoftExtract = $MicrosoftExtractDirs.FullName | Split-Path -Parent | Select-Object -First 1
												global:Write-CMLogEntry -Value "$($Product): Microsoft driver source directory set to $MicrosoftExtract" -Severity 1
										...
										if ($ImportInto -notmatch "Download") {
											global:Write-CMLogEntry -Value "$($Product): Checking for extracted drivers" -Severity 1
											if ($ImportInto -like "*Driver*") {
												if ((Get-ChildItem -Recurse -Path "$DriverExtractDest" -Filter *.inf -File).count -ne 0) {
													global:Write-CMLogEntry -Value "$($Product): Driver count in path $DriverExtractDest - $((Get-ChildItem -Recurse -Path "$DriverExtractDest" -Filter *.inf -File).count) " -Severity 1
													global:Write-CMLogEntry -Value "==================== $PRODUCT DRIVER IMPORT ====================" -Severity 1
													Set-Location -Path ($SiteCode + ":")
													Set-Location -Path $global:TempDirectory
													...
													global:Write-CMLogEntry -Value "$($Product): Creating driver package $CMDriverPackage" -Severity 1
													global:Write-CMLogEntry -Value "$($Product): Searching for driver INF files in $DriverExtractDest" -Severity 1
													...
													if ($DriverINFFiles.Count -ne $null) {
														...
														try {
															if (Get-CMCategory -CategoryType DriverCategories -name $DriverCategoryName) {
																global:Write-CMLogEntry -Value "$($Product): Category already exists" -Severity 1
																$DriverCategory = Get-CMCategory -CategoryType DriverCategories -name $DriverCategoryName 
															}
															else {
																global:Write-CMLogEntry -Value "$($Product): Creating category $DriverCategoryName" -Severity 1
																$DriverCategory = New-CMCategory -CategoryType DriverCategories -name $DriverCategoryName 
															}
															global:Write-CMLogEntry -Value "$($Product): Creating driver package for $Make $Model (Version $DriverRevision)" -Severity 1
															New-CMDriverPackage -Name $CMDriverPackage -Path "$DriverPackageDest"
															global:Write-CMLogEntry -Value "$($Product): New driver package name: $CMDriverPackage | Path $DriverPackageDest" -Severity 1
														...
														global:Write-CMLogEntry -Value "$($Product): Running driver import process (this might take several minutes)" -Severity 1
														if ($SCCMDriverPackage.PackageID -ne $null) {
															...
															try {
																...
																foreach ($DriverINF in $DriverINFFiles) {
																	...
																	global:Write-CMLogEntry -Value "$($Product): Importing driver INF $DriverNo of $($DriverINFFiles.count): $($DriverINF.FullName | Split-Path -Leaf)" -Severity 1
																	Add-CMDriverToDriverPackage -DriverID $DriverInfo.CI_ID -DriverPackageName "$($CMDriverPackage)"
																	$DriverNo++
																}
																global:Write-CMLogEntry -Value "$($Product): Driver package $($SCCMDriverPackage.PackageID) created succesfully" -Severity 1
																# =============== Distrubute Content =================
																global:Write-CMLogEntry -Value "$($Product): Distributing $($SCCMDriverPackage.PackageID)" -Severity 1
																DistributeContent -Product $Product -PackageID $SCCMDriverPackage.PackageID -ImportInto $ImportInto
															}
															catch {
																ErrorOutput -Message "Error: $($_.Exception.Message)" -Severity 3
															}
															...
															ErrorOutput -Message "Error: Errors occurred while creating driver package" -Severity 3
														}
														Set-Location -Path $global:TempDirectory
													}
													else {
														global:Write-CMLogEntry -Value "$($Product): Extract folder empty.. Skipping driver import / package creation" -Severity 2
													}
													Set-Location -Path $global:TempDirectory
												}
												else {
													global:Write-CMLogEntry -Value "======== DRIVER EXTRACT ISSUE DETECTED ========" -Severity 3
													global:Write-CMLogEntry -Value "$($Product): Issues occurred while reading extracted drivers" -Severity 3
													global:Write-CMLogEntry -Value "$($Product): Driver count in path $DriverExtractDest - $((Get-ChildItem -Recurse -Path "$DriverExtractDest" -Filter *.inf -File).count) " -Severity 1
												}
											}
											if ($ImportInto -like "*Standard*") {
												...
												if ((Get-ChildItem -Recurse -Path "$DriverExtractDest" -Filter *.inf -File).Count - $null) {
													global:Write-CMLogEntry -Value "$($Product): Validated drivers exist in $DriverExtractDest - Processing driver packaging steps " -Severity 1
													global:Write-CMLogEntry -Value "==================== $PRODUCT DRIVER PACKAGE  ====================" -Severity 1
													...
													if ([string]::IsNullOrEmpty($ExistingPackageID)) {
														...
														$PackageDescription = "$Make $Model Windows $WindowsVersion $Architecture Drivers (Models included:$SkuValue) "
														
														# Move Extracted Drivers To Driver Package Directory
														global:Write-CMLogEntry -Value "$($Product): Source directory $DriverExtractDest" -Severity 1
														global:Write-CMLogEntry -Value "$($Product): Destination directory $DriverPackageDest" -Severity 1
														...
														while ((Get-Job -Name "$Model-Driver-Package").State -eq "Running") {
															global:Write-CMLogEntry -Value "$($Product): Copying $Make $Model $OperatingSystem $Architecture Drivers.. Next check in 30 seconds" -Severity 1
															Start-Sleep -seconds 30
														}
														...
														if ((Get-Job -Name "$Model-Driver-Package").State -eq "Completed") {
															global:Write-CMLogEntry -Value "$($Product): Creating Package for $Make $Model (Version $DriverRevision)" -Severity 1
															Set-Location -Path ($SiteCode + ":")
															...
															# Check For Driver Package
															$SCCMPackage = Get-CMPackage -Name $CMPackage | Select-Object PackageID, Version, Name | Where-Object {
																$_.Version -eq $DriverRevision
															}
															Move-CMObject -FolderPath $global:VendorDriverFolder -ObjectID $SCCMPackage.PackageID
															global:Write-CMLogEntry -Value "$($Product): Checking for driver package $CMPackage with version number $DriverRevision" -Severity 1
															if ($SCCMPackage.PackageID -ne $null) {
																global:Write-CMLogEntry -Value "$($Product): Driver package $($SCCMPackage.PackageID) created succesfully" -Severity 1
																if ($EnableBinaryDifCheckBox.Checked -eq $true) {
																	global:Write-CMLogEntry -Value "$($Product): Enabling Binary Delta Replication" -Severity 1
																	Set-CMPackage -ID $SCCMPackage.PackageID -EnableBinaryDeltaReplication $true -Priority $DistributionPriorityCombo.Text
																}
																# =============== Distrubute Content =================
																DistributeContent -Product $Product -PackageID $SCCMPackage.PackageID -ImportInto $ImportInto
															}
															else {
																ErrorOutput -Message "Error: Errors occurred while creating package" -Severity 3
															}
														}
														else {
															ErrorOutput -Message "Error: Errors occurred while copying drivers" -Severity 3
														}
														Get-Job -Name "$Model-Driver-Package" | Remove-Job
														Set-Location -Path $global:TempDirectory
													}
													else {
														global:Write-CMLogEntry -Value "$($Product): Driver package already exists (Package ID: $($ExistingPackageID.PackageID))." -Severity 2
														Set-Location -Path ($SiteCode + ":")
														if (($ExistingPackageID.Description -notcontains "(Models included:") -eq $true) {
															Set-CMPackage -ID $ExistingPackageID.PackageID -Description "$Make $Model Windows $WindowsVersion $Architecture Drivers (Models included:$SkuValue)"
															global:Write-CMLogEntry -Value "$($Product): Updating driver package description to include system model ID $SkuValue" -Severity 1
															Set-Location -Path $global:TempDirectory
														}
													}
												}
										...
										elseif ($ImportInto -match "XML") {
											...
											Write-XMLModels -XMLPath $DownloadPath -Make $Make -Model $Model -MatchingValues $([string]$SkuValue) -OperatingSystem $OSComboBox.SelectedItem -Architecture $ArchitectureComboxBox.SelectedItem -Platform "XML"
											
											global:Write-CMLogEntry -Value "======== DRIVER FALLBACK FOLDERS ========" -Severity 1
											...
											foreach ($OS in $($OSComboBox.Items)) {
												if (!(Test-Path -Path (Join-Path -Path $DownloadPath -ChildPath "Fallback\$OS"))) {
													global:Write-CMLogEntry -Value "$($Product): Creating $OS driver fallback folder" -Severity 1
													New-Item -Path (Join-Path -Path $DownloadPath -ChildPath "Fallback\$OS") -Type dir
													foreach ($FallbackArchitecture in "x64", "x86") {
														if (!(Test-Path -Path (Join-Path -Path $DownloadPath -ChildPath "Fallback\$OS\$FallbackArchitecture"))) {
															global:Write-CMLogEntry -Value "$($Product): Creating $OS $FallbackArchitecture subfolder" -Severity 1
															New-Item -Path (Join-Path -Path $DownloadPath -ChildPath "Fallback\$OS\$FallbackArchitecture") -Type dir
														}
													}
												}
											}
								...
								elseif ($DriverDownload -eq "badLink") {
									global:Write-CMLogEntry -Value "$($Product): Operating system driver package download path not found.. Skipping $Model" -Severity 3
								}
								else {
									global:Write-CMLogEntry -Value "$($Product): Driver package not found for $Model running Windows $WindowsVersion $Architecture. Skipping $Model" -Severity 2
								}
							}
							else {
								global:Write-CMLogEntry -Value "$($Product): Driver package ($($ExistingPackageID.Name) - $($ExistingPackageID.Version) ($($ExistingPackageID.PackageID))) already exists." -Severity 1
								
							}
							global:Write-CMLogEntry -Value "======== $PRODUCT $MODEL DRIVER PROCESSING FINISHED ========" -Severity 1
						}
						Set-Location -Path $global:TempDirectory
					}
				}
				...
				if (($ImportInto -like "*Both*") -or ($ImportInto -eq "MDT")) {
					...
					# Import MDT Module
					$Product = "MDT"
					global:Write-CMLogEntry -Value "======== $Product Prerequisites ========" -Severity 1
					global:Write-CMLogEntry -Value "$($Product): Importing MDT PowerShell Module" -Severity 1
					...
					If ((Test-Path -Path $MDTPSLocation) -eq $true) {
						...
						# =================== MDT Driver Download =====================
						global:Write-CMLogEntry -Value "========  $Product Driver Download ========" -Severity 1
						global:Write-CMLogEntry -Value "$($Product): Starting $Product driver download process" -Severity 1
						# =================== DEFINE VARIABLES =====================					
						global:Write-CMLogEntry -Value "$($Product): Driver package base location set to $DownloadRoot" -Severity 1
						# Operating System Version
						$OperatingSystem = ("Windows " + $WindowsVersion)
						...
						if (($ModelURL -ne $null) -and ($ModelURL -ne "badLink")) {
							...
							if ((Test-Path -Path ($DownloadRoot + $Model + "\Driver Cab\" + $DriverCab)) -eq $false) {
								...
								global:Write-CMLogEntry -Value "$($Product): Skipping $DriverCab... Driver pack already downloaded" -Severity 2
							}
							# Check for destination directory, create if required and download the driver cab
							if ((Test-Path -Path "$($DownloadRoot + $Model + '\Driver Cab\' + $DriverCab)") -eq $false) {
								if ((Test-Path -Path "($DownloadRoot + $Model + '\Driver Cab\')") -eq $false) {
								...
								global:Write-CMLogEntry -Value "$($Product): Skipping $DriverCab.. Driver pack already extracted" -Severity 2
							}
							...
							if (((Test-Path -Path "$($DownloadRoot + $Model + '\Driver Cab\' + $DriverCab)") -eq $true) -and ($DriverCab -ne $null)) {
								...
								if ((Get-ChildItem -Path "$DriverExtractDest" -Recurse -Filter *.inf -File).Count -eq 0) {
									...
									if ($Make -eq "Hewlett-Packard") {
										...
										global:Write-CMLogEntry -Value "$($Product): Extracting HP drivers to $HPTemp" -Severity 1
										$HPSilentSwitches = "-PDF -F" + $HPTemp + " -S -E"
										global:Write-CMLogEntry -Value "$($Product): Using $Make silent switches: $HPSilentSwitches" -Severity 1
										global:Write-CMLogEntry -Value "$($Product): Extracting $Make drivers to $DriverExtractDest" -Severity 1
										Start-Process -FilePath "$($DownloadRoot + $Model + '\Driver Cab\' + $DriverCab)" -ArgumentList $HPSilentSwitches -Verb RunAs
								...
								global:Write-CMLogEntry -Value "$($Product): Error downloading $DriverCab" -Severity 3
							}
						}
					}
					else {
						ErrorOutput -Message "Error: MDT PowerShell Commandlets not found - Path specified $MDTPSLocation" -Severity 3
					}
					
					global:Write-CMLogEntry -Value "======== $PRODUCT $MODEL PROCESSING FINISHED ========" -Severity 1
				}
				
				# Remove legacy driver packages
				if ($RemoveLegacyDriverCheckbox.Checked -eq $true) {
					Set-Location -Path ($SiteCode + ":")
					global:Write-CMLogEntry -Value "======== Superseded Driver Package Option Processing ========" -Severity 1
					...
					if ($ModelDriverPacks.Count -gt "1") {
						foreach ($DriverPackage in $ModelDriverPacks) {
							if ($DriverPackage.PackageID -ne $LatestDriverPack.PackageID) {
								global:Write-CMLogEntry -Value "$($Product): Removing $($DriverPackage.Name) / Package ID $($DriverPackage.PackageID)" -Severity 1
					...
					if ($ModelDriverPacks.Count -gt "1") {
						...
						global:Write-CMLogEntry -Value "$($Product): Removing $($LegacyDriverPack.Name) / Package ID $($LegacyDriverPack.PackageID)" -Severity 1
						Remove-CMDriverPackage -id $LegacyDriverPack.PackageID -Force
					}
					$ModelDriverPackages = Get-CMPackage -Name "Drivers -*$Model*$WindowsVersion*$Architecture*" | Select-Object Name, PackageID, Version, SourceDate | Sort-Object SourceDate -Descending
				...
				if ($RemoveLegacyBIOSCheckbox.Checked -eq $true) {
					Set-Location -Path ($SiteCode + ":")
					global:Write-CMLogEntry -Value "======== Superseded BIOS Package Option Processing ========" -Severity 1
					$ModelBIOSPackages = Get-CMPackage -Name "BIOS Update*$Model" | Select-Object Name, PackageID, Version, SourceDate | Sort-Object SourceDate -Descending
					...
					if ($ModelBIOSPackages.Count -gt "1") {
						foreach ($BIOSPackage in $ModelBIOSPackages) {
							if ($BIOSPackage.PackageID -ne $LatestBIOSPackage.PackageID) {
								global:Write-CMLogEntry -Value "$($Product): Removing $($BIOSPackage.Name) / Package ID $($BIOSPackage.PackageID)" -Severity 1
				...
				$RemainingModels--
				global:Write-CMLogEntry -Value "Info: Remaining models to process: $RemainingModels" -Severity 1
			}
		}
		if ($ValidationErrors -eq 0) {
			if (($CleanUnusedCheckBox.Checked -eq $true) -or ($RemoveDriverSourceCheckbox.Checked -eq $true)) {
				global:Write-CMLogEntry -Value "======== Clean Up Driver Option Processing ========" -Severity 1
				if ($CleanUnusedCheckBox.Checked -eq $true) {
					Set-Location -Path ($SiteCode + ":")
					# Sleep to allow for driver package registration
					Start-Sleep -Seconds 10
					# Get list of unused drivers
					global:Write-CMLogEntry -Value "$($Product): Building driver list" -Severity 1
					$DriverList = Get-CMDriverPackage | Get-CMDriver | Select-Object -Property CI_ID
					global:Write-CMLogEntry -Value "$($Product): Building boot image driver list" -Severity 1
					...
					global:Write-CMLogEntry -Value "$($Product): Found $($UnusedDrivers.Count) unused drivers" -Severity 1
					global:Write-CMLogEntry -Value "$($Product): Starting driver package clean up process" -Severity 1
					foreach ($Driver in $UnusedDrivers) {
						global:Write-CMLogEntry -Value "$($Product): Removing $($Driver.LocalizedDisplayName) from category $($Driver.LocalizedCategoryInstanceNames)" -Severity 1
						Remove-CMDriver -ID $Driver.CI_ID -Force
					}
					global:Write-CMLogEntry -Value "$($Product): Driver clean up process completed" -Severity 1
					Set-Location -Path $global:TempDirectory
				}
				if ($RemoveDriverSourceCheckbox.Checked -eq $true) {
					...
					if ((($DownloadPathTextBox.Text) -ne $null) -and ((Test-Path -Path ($DownloadPathTextBox.text)) -eq $true)) {
						global:Write-CMLogEntry -Value "$($Product): Removing downloaded driver files from $($DownloadPathTextBox.Text).Extracted drivers will remain" -Severity 1
						Get-ChildItem -Path $($DownloadPathTextBox.Text) -Recurse -Directory | Where-Object {
							$_.FullName -match "Driver Cab"
			...
			if ($global:HPSoftPaqDownloads.Count -ge "1") {
				...
				foreach ($HPSoftPaqDownload in $global:HPSoftPaqDownloads.Keys) {
					global:Write-CMLogEntry -Value "======== HP Softpaq Download ========" -Severity 1
					# =================== DEFINE VARIABLES =====================
					...
					$HPSoftPaqVersion = ($HPSoftPaqDownload.Split("[") | Select -Last 1).TrimEnd("]")
					...
					if ($ImportInto -match "ConfigMgr") {
						...
						Set-Location -Path ($SiteCode + ":")
						$Product = "ConfigMgr"
						global:Write-CMLogEntry -Value "$($Product): Checking for pre-existing driver package" -Severity 1
					...
					if (($ImportInto -match "Download") -or ([string]::IsNullOrEmpty($($ExistingPackageID.PackageID)))) {
						...
						if ((Test-Path -Path $("$DownloadRoot" + "SoftPaqs\" + "$HPSoftPaqName\" + "$HPDriverCab")) -eq $false) {
							global:Write-CMLogEntry -Value "$($Product): Downloading $HPSoftPaqDownload" -Severity 1
							global:Write-CMLogEntry -Value "$($Product): Version is $HPSoftPaqVersion" -Severity 1
							global:Write-CMLogEntry -Value "$($Product): Creating $HPSoftPaqName download folder" -Severity 1
							if ((Test-Path -Path $("$DownloadRoot" + "SoftPaqs\" + "$HPSoftPaqName\")) -eq $false) {
								global:Write-CMLogEntry -Value "$($Product): Creating $("$DownloadRoot" + "SoftPaqs\" + $HPSoftPaqName) folder " -Severity 1
								New-Item -ItemType Directory -Path $("$DownloadRoot" + "SoftPaqs\" + $HPSoftPaqName)
							}
							global:Write-CMLogEntry -Value "$($Product): Downloading $HPDriverCab driver cab file" -Severity 1
							global:Write-CMLogEntry -Value "$($Product): Downloading from URL: $HPDriverDownload" -Severity 1
							Start-Job -Name "$HPSoftPaqName-SoftPaqDownload" -ScriptBlock $DriverDownloadJob -ArgumentList ($DownloadRoot, $Model, $HPDriverCab, $HPDriverDownload, $global:BitsProxyOptions, $global:BitsOptions, $global:ProxySettingsSet, $HPSoftPaqName, $SoftPaqDownload)
							Start-Sleep -Seconds 5
							...
							global:Write-CMLogEntry -Value "$($Product): Reported file byte size size: $global:BitsJobByteSize" -Severity 1
							global:Write-CMLogEntry -Value "$($Product): Downloaded file byte size:  $((Get-Item -Path $("$DownloadRoot" + "SoftPaqs\" + "$HPSoftPaqName\" + "$HPDriverCab")).Length)" -Severity 1
							$PreviousDownload = $false
						}
						else {
							global:Write-CMLogEntry -Value "$($Product): Skipping $HPDriverCab... Driver pack already downloaded." -Severity 1
							$PreviousDownload = $true
						}
						...
						if ((Test-Path -Path $("$DownloadRoot" + "SoftPaqs\" + "$HPSoftPaqName\" + "$HPDriverCab")) -eq $true) {
							...
							global:Write-CMLogEntry -Value "$($Product): Extracting $HPSoftPaqName SoftPaq to $HPSoftPaqTemp" -Severity 1
							$HPSoftPaqSilentSwitches = "-PDF -F" + "$HPSoftPaqTemp" + " -S -E"
							global:Write-CMLogEntry -Value "$($Product): Using $Make silent switches: $HPSoftPaqSilentSwitches" -Severity 1
							...
							global:Write-CMLogEntry -Value "$($Product): HP Extract location is $HPSoftPaqTemp" -Severity 1
							# Set HP SoftPaq destination
							$DriverExtractDest = ("$DownloadRoot" + "SoftPaqs" + "\" + "$HPSoftPaqName" + "\" + "$HPSoftPaqVersion")
							global:Write-CMLogEntry -Value "$($Product): HP driver source directory set to $DriverExtractDest" -Severity 1
							...
							if ((Test-Path -Path "$HPSoftPaqTemp") -eq $true) {
								...
								while ((Get-Job -Name "$HPSoftPaqName-Driver-Move").State -eq "Running") {
									global:Write-CMLogEntry -Value "$($Product): Moving $Make $HPSoftPaqName driver.. Next check in 30 seconds" -Severity 1
									Start-Sleep -seconds 30
								}
								...
								ErrorOutput -Message "Error: Issues occurred during the $Make $HPSoftPaqName extract process" -Severity 3
							}
							...
							if ($ExtractedFiles.Count -eq $ExtractedMovedFiles.Count) {
								if (($ImportInto -match "ConfigMgr") -or ($ImportInto -match "Both")) {
									...
									if ([string]::IsNullOrEmpty($ExistingPackageID.PackageID)) {
										...
										global:Write-CMLogEntry -Value "$($Product): Destination directory $DriverPackageDest" -Severity 1
										# Copy Drivers To Package Location
										Start-Job -Name "$HPSoftPaqName-Driver-Package" -ScriptBlock $PackageDrivers -ArgumentList ($Make, $DriverExtractDest, $Architecture, $DriverPackageDest)
										while ((Get-Job -Name "$HPSoftPaqName-Driver-Package").State -eq "Running") {
											global:Write-CMLogEntry -Value "$($Product): Copying $Make $HPSoftPaqName Drivers.. Next check in 30 seconds" -Severity 1
											Start-Sleep -seconds 30
										}
										...
										if ((Get-Job -Name "$HPSoftPaqName-Driver-Package").State -eq "Completed") {
											global:Write-CMLogEntry -Value "$($Product): Creating Package for $Make $HPSoftPaqName (Version $HPSoftPaqVersion)" -Severity 1
											Set-Location -Path ($SiteCode + ":")
											# Create Driver Package
											...
											global:Write-CMLogEntry -Value "$($Product): Saving package to $global:VendorDriverFolder folder" -Severity 1
											Move-CMObject -FolderPath $global:VendorDriverFolder -ObjectID $SCCMPackage.PackageID
											global:Write-CMLogEntry -Value "$($Product): Checking for driver package $CMPackage with version number $HPSoftPaqVersion" -Severity 1
											if (![string]::IsNullOrEmpty($SCCMPackage.PackageID)) {
												global:Write-CMLogEntry -Value "$($Product): Driver package $($SCCMPackage.PackageID) created succesfully" -Severity 1
												if ($EnableBinaryDifCheckBox.Checked -eq $true) {
													global:Write-CMLogEntry -Value "$($Product): Enabling Binary Delta Replication" -Severity 1
													Set-CMPackage -ID $SCCMPackage.PackageID -EnableBinaryDeltaReplication $true -Priority $DistributionPriorityCombo.Text
									...
									global:Write-CMLogEntry -Value "$($Product): Driver extraction task complete" -Severity 1
								}
							}
							else {
								global:Write-CMLogEntry -Value "$($Product): Issued occured during the extract process. Missing files, expecting $($ExtractedFiles.Count) and found $($ExtractedMovedFiles.Count). Skipping packaging steps" -Severity 1
							}
						}
						else {
							ErrorOutput -Message "Error: Driver package download issues occured" -Severity 1
						}
						Set-Location -Path $global:TempDirectory
					}
					else {
			...
			$ProgressBar.Increment(1)
			$ModelProgressOverlay.Increment(1)
			global:Write-CMLogEntry -Value "======== FINISHED PROCESSING ========" -Severity 1
		}
		if ($ValidationErrors -gt 0) {
			global:Write-CMLogEntry -Value "======== Validation Error(s) ========" -Severity 3
			global:Write-CMLogEntry -Value "Validation errors have occurred. Please review the log located at $global:LogFilePath." -Severity 3
		}
	}
	...
	function ScheduleDownloads {
		if ((Get-ScheduledTask | Where-Object {
			...
			global:Write-CMLogEntry -Value "======== Scheduling Job ========" -Severity 1
			global:Write-CMLogEntry -Value "Scheduling: Copying PowerShell script to $($ScriptLocation.Text)" -Severity 1
			...
			global:Write-CMLogEntry -Value "Scheduling: Creating Driver Automation Tool scheduled task" -Severity 1
			...
			global:Write-CMLogEntry -Value "WARNING: Scheduled task already exists." -Severity 2
		}
	}
	...
	function TestCredentials {
		try {
			...
			if ($CurrentDomain -ne $null) {
				...
				global:Write-CMLogEntry -Value "Non Domain environment: Testing local username / password" -Severity 2
		...
		{
			ErrorOutput -Message "Error: Username / Password incorrect" -Severity 3
			Return $false
		}
	}
	...
	function ValidateProxyAccess {
		...
		global:Write-CMLogEntry -Value "======== PROXY SERVER VALIDATION ========" -Severity 1
		...
		global:Write-CMLogEntry -Value "Proxy: Proxy server set to $ProxyServer" -Severity 1
		
		Try {
			global:Write-CMLogEntry -Value "Proxy: Testing authenticated proxy server access to $URL" -Severity 1
			$Content = $WebClient.DownloadString("http://" + $($URL.Host))
			global:Write-CMLogEntry -Value "Proxy: Connected to $URL successfully" -Severity 1
			...
			global:Write-CMLogEntry -Value "Proxy: Global proxy settings set for web/bits transfers" -Severity 1
		}
		catch [System.Exception] {
			global:Write-CMLogEntry -Value "Proxy: Unable to access URL: $URL. Error message: $($_.Exception.Message)" -Severity 3
		}
	}
	
	function ValidateMDT {
		$MDTDeploymentShareNames.Clear()
		$DeploymentShareGrid.Rows.Clear()
		$ProgressListBox.ForeColor = 'Black'
		global:Write-CMLogEntry -Value "======== Validating MDT PS Script Availability ========" -Severity 1
		if ($MDTScriptTextBox.Text -ne $MDTPSCommandlets) {
			global:Write-CMLogEntry -Value "Info: Using alternative location for MDT PowerShell cmdlets" -Severity 1
		...
		if ((Test-Path -Path $MDTPSCommandlets) -eq $true) {
			$MDTScriptTextBox.BackColor = 'White'
			global:Write-CMLogEntry -Value "Info: Setting MDT PS module path to default value." -Severity 1
			$MDTScriptTextBox.Text = "$MDTPSCommandlets"
			$MDTPSLocation = $MDTPSCommandlets
			try {
				global:Write-CMLogEntry -Value "Info: Importing MDT PS cmdlets" -Severity 1
				Import-Module "$MDTPSLocation"
				global:Write-CMLogEntry -Value "Info: Discovering MDT deployment shares" -Severity 1
			...
			$ProgressListBox.ForeColor = 'Maroon'
			global:Write-CMLogEntry -Value "======== MDT Issue Detected ========" -Severity 1
			global:Write-CMLogEntry -Value "Warning: Failed to locate MDT PS module. Please specify location on the MDT Settings tab." -Severity 2
	...
	function UpdateConfigMgrPkgList {
		if (($PackageTypeCombo.Text -ne $null) -and ($DeploymentStateCombo.Text -ne $null)) {
			try {
				...
				$ConfigMgrPkgs = Get-CMPackage -Name "$PackagePrefix -*" | Select-Object Name, PackageID, Version, SourceDate | Sort-Object Name
	...
	function MoveConfigMgrPkgs {
		...
		try {
			...
			global:Write-CMLogEntry -Value "======== Package State Change Processing ========" -Severity 1
			Do {
				for ($Row = 0; $Row -lt $PackageGrid.RowCount; $Row++) {
					if ($PackageGrid.Rows[$Row].Cells[0].Value -eq $true) {
						global:Write-CMLogEntry -Value "Info: Migrating package ID $($PackageGrid.Rows[$Row].Cells[3].Value) to driver $($ConfigMgrPkgActionCombo.Text) state" -Severity 1
						$CurrentState = ($PackageGrid.Rows[$Row].Cells[1].Value).split("-")[0]
						$CurrentPkgName = $($PackageGrid.Rows[$Row].Cells[1].Value)
						global:Write-CMLogEntry -Value "Info: Working with package $($PackageGrid.Rows[$Row].Cells[1].Value)" -Severity 1
						global:Write-CMLogEntry -Value "Info: Updating package ID $($PackageGrid.Rows[$Row].Cells[3].Value) to $State" -Severity 1
						...
						global:Write-CMLogEntry -Value "Info: Updating package name to $NewPackageName" -Severity 1
						Get-CMPackage -ID ($PackageGrid.Rows[$Row].Cells[3].Value) | Set-CMPackage -NewName $NewPackageName
	...
	function Create-CustomPkg {
		...
		$ConfigMgrPkg = {
			...
			$CMPackage = ("Drivers - " + "$Make " + $Model + " - " + $OperatingSystem + " " + $Architecture)
			global:Write-CMLogEntry -Value "Info: Copying source files to package directory" -Severity 1
			Set-Location -Path ($SiteCode + ":")
			$ExistingPackageID = (Get-CMPackage -Name $CMPackage | Select-Object PackageID, Name, Version | Where-Object {
					$_.Version -eq $DriverRevision
				})
			if ([string]::IsNullOrEmpty($ExistingPackageID)) {
				...
				if ((Test-Path -Path $DriverPackageDest) -eq $false) {
					try {
						global:Write-CMLogEntry -Value "Info: Creating driver package destination directory at $DriverPackageDest" -Severity 1
						New-Item -Path $DriverPackageDest -ItemType Dir
						global:Write-CMLogEntry -Value "Info: Copying source files to package directory" -Severity 1
						Copy-Item -Path $PackageSource -Destination $DriverPackageDest -Recurse
						Set-Location -Path ($SiteCode + ":")
						New-CMPackage -Name "$CMPackage" -path "$DriverPackageDest" -Manufacturer $Make -Description "$Make $Model Windows $WindowsVersion $Architecture Drivers (Models included:$SystemSKU)" -Version $DriverRevision
						$CustomPackage = Get-CMPackage -Name "$CMPackage" | Select-Object PackageID, Name, Version | Where-Object {
							$_.Version -eq $DriverRevision
						}
						global:Write-CMLogEntry -Value "Info: Package created $($CustomPackage.PackageID)" -Severity 1
						DistributeContent -Product $Platform -PackageID $CustomPackage.PackageID -ImportInto "Standard"
						global:Write-CMLogEntry -Value "Info: Distributing package $($CustomPackage.PackageID)" -Severity 1
					}
					catch [System.Exception] {
					...
					global:Write-CMLogEntry -Value "Warning: Package destination directory already exists." -Severity 2
					global:Write-CMLogEntry -Value "Remove files at $DriverPackageDest folder to replace this package" -Severity 2
				}
			}
			else {
				global:Write-CMLogEntry -Value "Info: Package already exists (Package ID: $($ExistingPackageID.PackageID))." -Severity 1
			}
			Set-Location -Path $global:TempDirectory
		}
		...
		$XMLPkg = {
			...
			try {
				if ((Test-Path -Path $DriverPackageDest) -eq $false) {
					global:Write-CMLogEntry -Value "$($Platform): Copying drivers to package directory - $PackageSource" -Severity 1
		...
		global:Write-CMLogEntry -Value "======== Processing Custom Packages ========" -Severity 1
		Do {
											...
											if (![string]::IsNullOrEmpty($CustomPkgDataGrid.Rows[$Row].Cells["Revision"].Value)) {
												...
												if (![string]::IsNullOrEmpty($CustomPkgDataGrid.Rows[$Row].Cells["SourceDirectory"].Value)) {
													...
													if ((Test-Path -Path "$PackageSource") -eq $true) {
														global:Write-CMLogEntry -Value "Info: Running $Platform import job for $Make $Model" -Severity 1
														...
														global:Write-CMLogEntry -Value "Warning: Source path does not exist or is not accessible." -Severity 2
													}
												}
												else {
													global:Write-CMLogEntry -Value "Warning: Package source field entry on row $Row is empty." -Severity 2
												}
											}
											else {
												global:Write-CMLogEntry -Value "Warning: Version field entry on row $Row is empty. " -Severity 2
											}
										}
										else {
											global:Write-CMLogEntry -Value "Warning: Architecture field entry on row $Row is empty." -Severity 2
										}
									}
									else {
										global:Write-CMLogEntry -Value "Warning: Operating System field entry on row $Row is empty." -Severity 2
									}
								}
								else {
									global:Write-CMLogEntry -Value "Warning: Platform field entry on row $Row is empty." -Severity 2
								}
							}
							else {
								global:Write-CMLogEntry -Value "Warning: Idenifier field entry on row $Row is empty." -Severity 2
							}
						}
						else {
							global:Write-CMLogEntry -Value "Warning: Make field entry on row $Row is empty." -Severity 2
						}
					}
					else {
						global:Write-CMLogEntry -Value "Warning: Model field entry on row $Row is empty." -Severity 2
					}
				}
	...
	function Import-CSVModels {
		...
		global:Write-CMLogEntry -Value "======== CSV Import Process ========" -Severity 1
		global:Write-CMLogEntry -Value "Info: Importing models from comma separated value source file" -Severity 1
		global:Write-CMLogEntry -Value "Info: CSV location - $CSVFileName" -Severity 1
		try {
			if ($CSVFileName -match ".csv") {
				$ModelsToImport = Import-Csv -Path $CSVFileName
				global:Write-CMLogEntry -Value "Info: $($ModelsToImport.Model.Count) models found" -Severity 1
										...
										if (($Model.Architecture -eq "x64") -or ($Model.Architecture -eq "x86")) {
											if (!([string]::IsNullOrEmpty($Model.Version))) {
												if ($Model.'Source Directory') {
													global:Write-CMLogEntry -Value "Info: All fields have been verified. Adding $($Model.Make) $($Model.Model) to list." -Severity 1
													...
													global:Write-CMLogEntry -Value "Warning: Source directory field is empty." -Severity 2
												}
											}
											else {
												global:Write-CMLogEntry -Value "Warning: Version field is empty." -Severity 2
											}
										}
										else {
											global:Write-CMLogEntry -Value "Warning: Architecture either incorrectly or not specified." -Severity 2
										}
									}
									else {
										global:Write-CMLogEntry -Value "Warning: Operating system either incorrectly or not specified." -Severity 2
									}
								}
								else {
									global:Write-CMLogEntry -Value "Warning: Produst must be specified as either ConfigMgr or MDT." -Severity 2
								}
							}
							else {
								global:Write-CMLogEntry -Value "Warning: Baseboard product field is empty." -Severity 2
							}
						}
						else {
							global:Write-CMLogEntry -Value "Warning: Model field is empty." -Severity 2
						}
					}
					else {
						global:Write-CMLogEntry -Value "Warning: Make field is empty." -Severity 2
					}
				}
				global:Write-CMLogEntry -Value "Info: Finished import process" -Severity 1
			}
		}
		catch [System.Exception] {
			ErrorOutput -Message "Error: $($_.Exception.Message)" -Severity 3
	...
	function Start-MDTImportProcess {
		...
		global:Write-CMLogEntry -Value "======== $PRODUCT Driver Import ========" -Severity 1
		global:Write-CMLogEntry -Value "$($Product): Starting MDT Driver Import Process" -Severity 1
		foreach ($MDTDeploymentShare in $global:MDTDeploymentShares) {
			# Detect First MDT PSDrive
			global:Write-CMLogEntry -Value "$($Product): Connecting MDT PSDrive $($MDTDeploymentShare.Cells["Name"].Value)" -Severity 1
			$PSDriveName = ($MDTDeploymentShare.Cells["Name"].Value)
			# Detect First MDT Deployment Share
			global:Write-CMLogEntry -Value "$($Product): Using MDT Deployment Path $($MDTDeploymentShare.Cells[1].Value)" -Severity 1
			...
			global:Write-CMLogEntry -Value "$($Product): Connecting to MDT share ($PSDriveName)" -Severity 1
			if (!(Get-PSDrive -Name $PSDriveName -ErrorAction SilentlyContinue)) {
				...
				global:Write-CMLogEntry -Value "$($Product): $PSDriveName connected to $DeploymentShare" -Severity 1
			}
			# Cater for HP / Model Issue
			$Model = $Model -replace '/', '-'
			...
			$Make = SetManufacturer -Make $Make
			# =============== MDT Driver Import ====================	
			$OperatingSystemDir = ($OperatingSystem.TrimEnd() + " " + $Architecture)
			global:Write-CMLogEntry -Value "$($Product): Creating $($MDTDriverStructureCombo.SelectedText) folder structure" -Severity 1
			global:Write-CMLogEntry -Value "$($Product): Creating $($MDTDriverStructureCombo.SelectedItem) folder structure" -Severity 1
			...
			switch -wildcard ($MDTDriverStructureCombo.SelectedItem) {
				"OperatingSystemDir\*" {
					...
					if (((Test-Path $MDTDriverPath\$OperatingSystemDir\$Make\$Model\$DriverRevision) -eq $false) -and ($($MDTDriverStructureCombo.SelectedItem) -match "Version")) {
						New-Item -path $MDTDriverPath\$OperatingSystemDir\$Make\$Model -enable "True" -Name $DriverRevision -ItemType Directory
					}
					if ($($MDTDriverStructureCombo.SelectedItem) -match "Version") {
				...
				"Make\*" {
					...
					if (((Test-Path $MDTDriverPath\$Make\$Model\$OperatingSystemDir\$DriverRevision) -eq $false) -and ($($MDTDriverStructureCombo.SelectedItem) -match "Version")) {
			...
			global:Write-CMLogEntry -Value "$($Product): Importing MDT driver pack for $Make $Model - Revision $DriverRevision" -Severity 1
			global:Write-CMLogEntry -Value "$($Product): MDT Driver Path = $MDTDriverPath" -Severity 1
			try {
				# =============== MDT Driver Import ====================				
				if ($Make -match "Dell") {
					...
					global:Write-CMLogEntry -Value "$($Product): Importing MDT Drivers from $DriverExtractDest. This might take several minutes." -Severity 1
					Import-MDTDriver -path "$MDTDriverPath" -SourcePath "$DriverExtractDest"
					global:Write-CMLogEntry -Value "$($Product): Dell Driver package added successfully" -Severity 1
				}
				else {
					global:Write-CMLogEntry -Value "$($Product): Importing MDT Drivers from $DriverExtractDest. This might take several minutes." -Severity 1
					Import-MDTDriver -path "$MDTDriverPath" -SourcePath "$DriverExtractDest"
					global:Write-CMLogEntry -Value "$($Product): Driver package added successfully" -Severity 1
				}
			}
			catch [system.Exception]{
				ErrorOutput -Message "Error: $($_.Exception.Message)" -Severity 3
	...
	function Get-MDTDeploymentShares {
		...
		global:Write-CMLogEntry -Value "======== $Product Deployment Share Info ========" -Severity 1
		global:Write-CMLogEntry -Value "Info: Found $($DeploymentShareGrid.Rows.Count) MDT deployment shares available" -Severity 1
		foreach ($MDTDeploymentShare in $global:MDTDeploymentShares) {
			global:Write-CMLogEntry -Value "Info: Adding MDT Deployment Share - $($MDTDeploymentShare.Cells[0].Value)" -Severity 1
			global:Write-CMLogEntry -Value "Info: Adding MDT Deployment Path - $($MDTDeploymentShare.Cells[1].Value)" -Severity 1
		}
		if ([string]::IsNullOrEmpty($global:MDTDeploymentShares)) {
			global:Write-CMLogEntry -Value "Warning: No MDT deployment shares have been selected" -Severity 2
		}
	}
	...
	function Create-DriverFBPkg {
		...
		try {
			...
			Set-Location -Path ($SiteCode + ":")
			$ExistingPackageID = Get-CMPackage -Name $CMPackage | Select-Object PackageID, Name, Version
			if ([string]::IsNullOrEmpty($ExistingPackageID)) {
				Set-Location -Path $global:TempDirectory
				if ((Test-Path -Path $DriverPackageDest) -eq $false) {
					try {
						...
						global:Write-CMLogEntry -Value "Info: Driver fallback package created $($CustomPackage.PackageID) for $WindowsVersion $Architecture" -Severity 1
						DistributeContent -Product $Platform -PackageID $CustomPackage.PackageID -ImportInto "Standard"
					}
					catch [System.Exception] {
						Write-Warning -Message "Error: Errors occurred while creating package - $($_.Exception.Message)"
					...
					global:Write-CMLogEntry -Value "Warning: Driver fallback package destination directory already exists." -Severity 2
				}
			}
			else {
				global:Write-CMLogEntry -Value "Info: Package already exists (Package ID: $($ExistingPackageID.PackageID))." -Severity 1
	...
	function Test-ConfigMgrWebSVC {
		...
		global:Write-CMLogEntry -Value "WebService response time - $($WebServiceResponse.Milliseconds)ms" -Severity 1
		$WebServiceResponseTime.Text = "$($WebServiceResponse.Milliseconds)ms"
		if ([string]::IsNullOrEmpty($WebServiceError)) {
			try {
				global:Write-CMLogEntry -Value "Connecting to the ConfigMgr WebService using URL - $URL" -Severity 1
				...
				global:Write-CMLogEntry -Value "WebService status version - $WebServiceBuild" -Severity 1
				$WebServiceVersion.Text = $WebServiceBuild
				global:Write-CMLogEntry -Value "WebService status code - $($WebServiceDetails.StatusCode)" -Severity 1
				$WebServiceStatusCode.Text = $WebServiceDetails.StatusCode
				global:Write-CMLogEntry -Value "WebService status description - $($WebServiceDetails.StatusDescription)" -Severity 1
				$WebServiceStatusDescription.Text = [string]$WebServiceDetails.StatusDescription
			}
			catch [System.Exception] {
				global:Write-CMLogEntry -Value "Errors occured while communicating with the ConfigMgr WebService. Error message: $($_.Exception.Message)" -Severity 3
			}
		}
		else {
			global:Write-CMLogEntry -Value "WebService status code - $($WebServiceError.Response.StatusCode.Value__)" -Severity 1
			$WebServiceStatusCode.Text = $WebServiceError.Response.StatusCode.Value__
			global:Write-CMLogEntry -Value "WebService status description - $($WebServiceError.Response.StatusDescription)" -Severity 1
		...
		try {
			...
			if (($WebServiceDriverPackages.Count -gt 0) -or ($WebServiceBIOSPackages.Count -gt 0)) {
				...
				global:Write-CMLogEntry -Value "Retrieved a total of $($WebServiceDriverPackages.Count) driver packages from web service" -Severity 1
				...
				global:Write-CMLogEntry -Value "Retrieved a total of $($WebServiceBIOSPackages.Count) BIOS packages from web service" -Severity 1
				...
				global:Write-CMLogEntry -Value "The ConfigMgr Webservice returned 0 packages. Please ensure that you have added packages and you are using the correct secret key." -Severity 1
			}
		}
		catch [System.Exception] {
			global:Write-CMLogEntry -Value "An error occured while calling ConfigMgr WebService for a list of available packages. Error message: $($_.Exception.Message)" -Severity 3
		}
	}
			...
			if ($MakeModelDataGrid.Rows[$Row].Cells[1].Value -match $SearchMake) {
				if ($SearchMake -ne "Hewlett-Packard") {
					if ($SearchList -contains $MakeModelDataGrid.Rows[$Row].Cells[2].Value) {
						global:Write-CMLogEntry -Value "Info: Selecting known model $($MakeModelDataGrid.Rows[$Row].Cells[2].Value)" -Severity 1
						$MakeModelDataGrid.Rows[$Row].Selected = $true
						$MakeModelDataGrid.Rows[$Row].Cells[0].Value = $true
	...
	function RegPreferences {
		...
		if (-not (Test-Path -Path $global:RegistryPath)) {
			global:Write-CMLogEntry -Value "======== CREATING REGISTRY ENTRIES ========" -Severity 1
	...
	function UpdateOSModelSuppport {
	...
	function FindAvailableModels {
		...
		if (($global:ProxySettingsSet -ne $true) -and ($UseProxyServerCheckbox.Checked -eq $true)) {
			ValidateProxyAccess -ProxyServer $ProxyServerInput.Text -UserName $ProxyUserInput.Text -Password $ProxyPswdInput.Text -URL "http://www.scconfigmgr.com"
		}
		$MakeModelDataGrid.Rows.Clear()
		UpdateModeList $SiteServerInput.Text $SiteCodeText.Text
		$ModelResults.Text = "Found ($($MakeModelDataGrid.Rows.Count)) matches"
	...
	function UpdatePlatformOptions {
		...
		elseif ($PlatformComboBox.SelectedItem -match "Standard") {
			...
			$RemoveLegacyDriverCheckbox.Enabled = $true
			$RemoveLegacyBIOSCheckbox.Enabled = $true
			MDTOptions -OptionsEnabled $false
			ConfigMgrOptions -OptionsEnabled $true
		}

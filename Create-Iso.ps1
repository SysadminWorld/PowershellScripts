...
# Set folder permissions on all folders so that we can create the ISO
$AllFolders = Get-ChildItem -path $DriveLetter -Recurse
Write-CMLogEntry -Value "Setting the ACLs so that we can capture the data." -Severity 1
ForEach ($Folder in $AllFolders )
{
    ...
    $acl | Set-Acl $Folder -WarningAction SilentlyContinue
}
...
# ISO File Name
If (!$Debug) { $FileName = $UAAComputerName }
If ($Debug) { $FileName = 'ISO-Test' }
...
$UserPath = "$StoreLocale\$($FileName)-$($($UserProfile).FullName).iso"
Write-CMLogEntry -Value "Set the full filename including path to $FullPath" -Severity 1

Write-CMLogEntry -Value 'Beginning creation of ISO file for backup' -Severity 1
#If (!$Debug) { (Get-ChildItem -Path $DriveLetter | Where { ($_.PsIsContainer -and $_.FullName -notmatch 'archive') -and ($_.PsIsContainer -and $_.FullName -notmatch 'Users')  } ) | New-IsoFile -Path $FullPath -Media BDRE -Force | Out-Null }
If (!$Debug)
{
    $UserProfiles = Get-ChildItem -Path "$($DriveLetter)Users" | Where { ($_.PsIsContainer -and $_.FullName -notmatch 'All Users') }
    ForEach ($UserProfile in $UserProfiles) { 
        $UserPath = "$StoreLocale\$($FileName)-$($($UserProfile).Name).iso"
        $UserProfile.FullName | New-IsoFile -Path $UserPath -Media DISK -Force | Out-Null
    }
}
    ...
    $systemdrive = Get-WmiObject Win32_DiskPartition |Where-Object {$_.BootPartition -eq "true"}
    [int]$DiskNumber = $($systemdrive.Name).Substring(6,1)

    If ($disks.Count -eq 2)
    {
        foreach($disk in $disks)
        {
             if($disk.Number -eq [int]$DiskNumber) {
                    #Write-Host "$($myInvocation.MyCommand) - Detected more than one disk - Disk #$($disk.Number)"
                    $TSenv.Value("DriveToImage") = $disk.Number
              }
              else
              {
                    $tsenv.Value("DriveToImage") = $disk.Number
